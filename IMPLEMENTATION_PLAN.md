...
| P7-S1c | Löschantrag-Workflow für reguläre Nutzer über Kontextmenü ("Löschung beantragen" statt direktem Löschen), inkl. Vier-Augen-Prinzip auch für Löschadmins (kein Alleingang). Neuer, separat konfigurierbarer Aktionstyp (z. B. `document.delete`/`folder.delete`) über das bestehende `permission-service`-Genehmigungsmuster (siehe `document.force_delete`, seit P7-S1) — bewusst getrennt von der bereits gebauten, Retention-ausgelösten Zwangslöschung. Zu klären: ob nur "endgültig löschen" oder bereits das Verschieben in den Papierkorb genehmigungspflichtig sein soll. *(Rückfrage bei P7-S1b-Planfreigabe, siehe `PROGRESS.md`.)* |
