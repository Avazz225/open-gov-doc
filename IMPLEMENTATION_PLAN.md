...
| P6-S9 | Federation Hub Service Grundgerüst: Adressbuch verbundener Installationen, externe Swimlanes im Process Designer, automatischer Handover bei Erreichen einer fremden Swimlane, Benachrichtigung/gemeinsames Design der Zielinstallation (7.4). **Vorgezogen von P13-S3** (Adressbuch/Schaltzentrale/Versionskompatibilität) **und in der Sache auch von P13-S4** (föderierte Workflow-Schritte — genau das, was externe Swimlanes/Handover technisch sind), Nutzerwunsch bei der P6-S8-Planfreigabe, siehe `PROGRESS.md`. Bewusst als eigene Session, da ein Federation Hub ein eigenständig betriebener, installationsübergreifender Vermittlungsdienst mit eigenem Vertrauens-/Sicherheitsmodell ist, kein Bestandteil einer einzelnen Installation. Bleibt trotzdem **keine Multi-Installation** des Process Designer selbst — jede Installation betreibt weiterhin ihren eigenen Container, nur der Handover läuft über den Hub. Die echte **Ende-zu-Ende-Verschlüsselung** zwischen Installationen bleibt vorerst bei P13-S4 (baut auf den erst in P13-S1 entstehenden Mehrfachinstallations-/Installations-ID-Grundlagen auf, die zu diesem frühen Zeitpunkt noch nicht existieren) — bei Sessionstart zu bestätigen, welches Trust-/Transport-Sicherheitsniveau ohne diese Grundlagen realistisch umsetzbar ist. |
...
| P13-S4 | Ende-zu-Ende-Verschlüsselung für die in P6-S9 vorgezogenen föderierten Workflow-Schritte (7.4) — die Handover-/Swimlane-Mechanik selbst existiert dann bereits, hier nur noch die kryptografische Absicherung auf Basis der inzwischen existierenden Installations-ID-Grundlagen (P13-S1). |
