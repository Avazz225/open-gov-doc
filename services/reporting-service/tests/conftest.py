...
async def _clean_tables():
    ...
    async with eng.begin() as conn:
        ...
        await conn.execute(text("TRUNCATE reporting.document_created_event"))
        await conn.execute(text("TRUNCATE reporting.report_schedule CASCADE"))
        await conn.execute(text("TRUNCATE reporting.report_run"))
    await eng.dispose()
    yield
