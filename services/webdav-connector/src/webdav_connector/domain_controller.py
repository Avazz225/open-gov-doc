...
class DmsAuthDomainController(BaseDomainController):
    ...
    def _resolve_edit_token(self, token: str) -> tuple[str, str] | None:
        ...
        instead of the real identity as `created_by`/lock holder. Also
        returns `document_id`, the one document this token was minted for -
        see `basic_auth_user`'s own comment on why this matters (ADR
        0189/P61-S4)."""
        try:
        ...
        if response.status_code != 200:
            return None
        body = response.json()
        return body["principal_id"], body["document_id"]
    ...
    def basic_auth_user(self, realm, user_name, password, environ):
        ...
        if not password:
            resolved = self._resolve_edit_token(user_name)
            if resolved is None:
                return False
            principal_id, document_id = resolved
            environ["wsgidav.auth.user_name"] = principal_id
            # ADR 0189/P61-S4: without this, resolving the token to the
            # REAL underlying user's identity above makes the rest of the
            # session indistinguishable from that user's real password
            # login - browsable/writable/deletable via any WebDAV client for
            # anything that principal has permission on, not just the one
            # document this token was minted for. `dav_provider.py`'s
            # `get_resource_inst` reads this to reject any path outside
            # `by-id/<document_id>` for a token-authenticated session.
            environ["dms.webdav_edit_token_document_id"] = document_id
            return True
