...
**Since P54-S1** ([ADR 0172](../adr/0172-fleet-management-service-operator-key-gate.md)): every endpoint except `/healthz` requires `Authorization: Bearer <fleet_operator_key>` — before this session, this service's ENTIRE API had no authentication of any kind (confirmed by a dedicated live-code security sweep, not a doc-flagged gap — nothing had previously documented this). `settings.fleet_operator_key` defaults to `None`, fully locking the API in every environment that doesn't explicitly configure it, including the dev stack — the same fail-closed `hub_operator_key` mechanism `federation-hub-service` already uses (ADR 0039/0162), reused rather than building a first admin-JWT/capability model just for this service. Unlike `hub_operator_key` (which only gates two of `federation-hub-service`'s many endpoints), this session gates the whole surface: every one of this service's endpoints returns sensitive data (`GET /installations` still enumerates every managed installation's `gateway_base_url`; `GET /plans`/`GET /rollouts` expose the fleet's entire update state) or performs a sensitive mutation (`rotate_installation_key` mints a fresh plaintext `fleet_agent_api_key`, the credential a real managed installation trusts). ~~No admin-UI currently calls this service (confirmed via grep of `apps/admin-ui/src/lib/api.ts` — no call site exists), so this remains a pure `curl`/operator-CLI-driven service with nothing on the frontend side to update.~~ — **closed in P67-S1** ([ADR 0197](../adr/0197-admin-ui-fleet-management-view.md)): `admin-ui`'s new `FleetManagementView` (`/fleet-management/`) now calls installation registration/listing/deletion/status/license-push directly from the browser. This surfaced that the service had no `CORSMiddleware` at all (never needed before, since no browser had ever called it) — now added, same origins/shape as `federation-hub-service`'s own (`settings.cors_allowed_origins`). While touching `_require_operator_key` for this, also switched its comparison to `hmac.compare_digest` (timing-safety fix `federation-hub-service`'s own equivalent check already got at P62-S1, never ported here). Group/plan/rollout management remain `curl`/operator-CLI-only — a separate, larger UI surface, not built in this session.
...
`services/fleet-management-service/tests/` — **33 tests since P54-S1** (+3: `test_installations_requires_fleet_operator_key`/`test_installations_with_wrong_operator_key_returns_403` — the new gate's regression proof, and `test_healthz_stays_ungated_without_operator_key` confirming the one deliberate exception — see [ADR 0172](../adr/0172-fleet-management-service-operator-key-gate.md)). The shared `client` fixture now sets `settings.fleet_operator_key` and applies the resulting bearer header as a `TestClient`-level default (not per call site) — every one of this service's tests needed the header once the whole API was gated, so a global fixture default was used instead of `federation-hub-service`'s own per-test set/reset pattern (ADR 0162), which only ever had to cover two endpoints. Before P54-S1, 30 tests (previously 26, +4 since **Post-Roadmap Phase 21 Session 1**, [ADR 0084](../adr/0084-fleet-license-key-rotation.md): default generation of a new key, operator-supplied value, actual use of the new value on an outgoing call, `404` for an unknown installation), before that 26 (since P13-S2b, previously 13), in-process `TestClient` against an ASGI stub of the target installation (`httpx.ASGITransport`, same pattern as `federation-hub-service`): installation CRUD including key-shown-only-once, status retrieval (reachable/unreachable), aggregation across multiple installations, license push, provisioning, 404 cases, as well as (P13-S2b) group membership, plan validation, rollout resolution (group/include/exclude, empty target set), a complete rollout happy path across all step types including four-eyes, `recoverable_failed`→`retry`, `fatal_contract`→`acknowledge-fatal`, `verify` for an unreachable installation → `retry_later`, rejection of an approval. The P13-S2 capabilities additionally verified live against the running Docker Compose stack (self-loopback). **Since P54-S1**: live-verified directly against the real, rebuilt container — every previously-open endpoint now returns `403` with no header and with a wrong key (confirmed for both a read, `GET /installations`, and the most sensitive write, `POST /installations`); a temporarily-configured `DMS_FLEET_OPERATOR_KEY` (added to `infra/docker-compose.yml`, then removed again immediately after verification) passed the gate and reached the real `200`; `/healthz` confirmed still reachable with no header throughout.
...
- No template library for `POST .../provision` — pure pass-through (confirmed still accurate: `provision_installation`'s own docstring states "this service itself does not curate a template library"). Since P17-S1 the forwarded document can carry an optional `manifest` (concept §14.1, `config-service`). ~~The first concrete, curated package (eGov, 14.2) is not assigned until P17-S2/S3~~ — **corrected during Phase 65+'s gap-analysis round**: P17-S2/S3 have long since shipped, `packages/egov/config.json` is the completed package, and its own README already documents this service as the intended application path ("Fleet Management: `POST /installations/{id}/provision` with the contents of this file as the body").
