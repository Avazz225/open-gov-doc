...
**Since P54-S1** ([ADR 0172](../adr/0172-fleet-management-service-operator-key-gate.md)): every endpoint except `/healthz` requires `Authorization: Bearer <fleet_operator_key>` — before this session, this service's ENTIRE API had no authentication of any kind (confirmed by a dedicated live-code security sweep, not a doc-flagged gap — nothing had previously documented this). `settings.fleet_operator_key` defaults to `None`, fully locking the API in every environment that doesn't explicitly configure it, including the dev stack — the same fail-closed `hub_operator_key` mechanism `federation-hub-service` already uses (ADR 0039/0162), reused rather than building a first admin-JWT/capability model just for this service. Unlike `hub_operator_key` (which only gates two of `federation-hub-service`'s many endpoints), this session gates the whole surface: every one of this service's endpoints returns sensitive data (`GET /installations` still enumerates every managed installation's `gateway_base_url`; `GET /plans`/`GET /rollouts` expose the fleet's entire update state) or performs a sensitive mutation (`rotate_installation_key` mints a fresh plaintext `fleet_agent_api_key`, the credential a real managed installation trusts). No admin-UI currently calls this service (confirmed via grep of `apps/admin-ui/src/lib/api.ts` — no call site exists), so this remains a pure `curl`/operator-CLI-driven service with nothing on the frontend side to update.
