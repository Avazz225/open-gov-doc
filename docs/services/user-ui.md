...
- **`ExplorerPane`** (oben links, Ansicht "Dokumente"): Windows-Explorer-artige Ordnernavigation mit Breadcrumb, Ordner-CRUD (Anlegen/Umbenennen/Löschen — die Backend-Endpunkte existierten bereits seit P3-S3, nur die UI dafür fehlte) und einer Tableiste für geöffnete Dokumente. Klick auf ein Dokument öffnet es als Tab, statt einen modalen Dialog zu zeigen. Seit P5b-S4 zusätzlich: nutzerseitig umschaltbare **Baum-/Listenansicht** (`explorer.viewModeList`/`viewModeTree`, in `localStorage` unter `dms.explorer.viewMode` gemerkt, keine geräteübergreifende Synchronisierung wie beim Farbschema — siehe "Offene Punkte") sowie das **Klassen-Icon** (2.2a/2.2b) vor jedem Ordnernamen in beiden Ansichten.
...
- **`UploadForm`** (Upload-Dialog in `ExplorerPane`): kannte vor P5b-S4 überhaupt keine Objekttypen (nur Datei + Titel). Seit dieser Session kann beim Hochladen eine **Dokumentklasse** gewählt werden (`GET /api/object-type-service/object-types?applies_to=document`); deren Attributfelder werden über das **"upload"-Formular-Layout** (2.2b) angeordnet, analog zu `MetadataPanel`/`SearchPane`. Die Objekttyp-Hierarchie-Prüfung (2.2a, `allowedParentTypes`) übernimmt weiterhin ausschließlich der Document Service serverseitig beim `POST /documents` — diese UI dupliziert diese Prüfung nicht, ein Verstoß zeigt einfach die vom Server gelieferte Fehlermeldung.
