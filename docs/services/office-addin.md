...
**Since Post-Roadmap Phase 75 Session 2** ([ADR 0219](../adr/0219-login-page-tailwind-redesign-accent-fg-token-cascade-fix.md)): `/login` rebuilt as a scaled-down card (smaller padding/type scale than the other five apps, no `useBranding` since this app has none), consistent with the reduced-theming exception. Found and fixed a real, second bug this session: this app's pre-existing `globals.css` has global `input[type="password"]`/`button` tag-selector rules that, being unlayered, silently beat the new `@layer utilities` Tailwind classes per the CSS cascade's layer-ordering rule — fixed by wrapping all of this app's existing hand-written CSS in `@layer base { ... }` (see the ADR). Verification used a standalone static probe (`getComputedStyle()` against the real compiled CSS, served via the static export) rather than the running app, since this app's Office.js host-gating (see above) blocks reaching `/login` in a plain browser even with `office.js` network-blocked.

**Since Post-Roadmap Phase 75 Session 3** ([ADR 0220](../adr/0220-office-addin-tailwind-rollout-primary-button-border-fix.md)): the rest of this app's UI (`Shell`, `DocumentPicker`, `TemplatePicker`, `MetadataForm`, `WorkflowPanel`, `TaskPane`, `OfficeGate`, `RequireAuth`) converted from hand-written CSS classes to Tailwind utilities — this app's own `globals.css` now holds only a minimal reset, no component-specific classes remain. The same `getComputedStyle()` static-probe technique caught a real defect this time: every `bg-accent` primary button (including this app's own, and — found and fixed the same session — all five other apps' login submit buttons) was silently relying on the browser's default unstyled `<button>` border, invisible in a screenshot but present in computed styles; fixed with an explicit `border-0`.
