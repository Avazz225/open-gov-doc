...
| `DELETE` | `/folders/{id}` | Immediate hard delete — 409 if subfolders still exist, or active documents (`document_client.count_active`, since **Phase 51 Session 3**, see "Open Points"), or if `inbox`/`outbox` (since P15-S3). Fallback for cases that are already empty and never subject to retention; the regular path since P7-S1b is `POST .../trash` |
...
- ~~**`DELETE /folders/{id}` (raw hard-delete fallback) only ever checked for contained subfolders, never contained documents** — `repository.delete_folder()`'s not-empty check is a purely local `list_children` query; documents live in `document-service`, a different service, and were never consulted at all on this path (unlike the regular `POST .../trash` path, which cascades onto documents via a synchronous `document_client.cascade_trash` call). A real orphaning risk: a folder with active documents could be hard-deleted directly, leaving those documents referencing a nonexistent `folder_id`. First named at P4-S4, re-surfaced during the Phase 44+ gap-analysis round (see `docs/services/user-ui.md`'s "Open Points").~~ — **closed in Phase 51 Session 3**: `delete_folder` now also calls `document_client.count_active([folder_id])` (the same client method the forced-deletion path already used) and rejects with `409` if it returns non-zero, before proceeding to the local-only `repository.delete_folder()` check. Live-verified against the real running stack: a folder with one active document now returns `409` on `DELETE`; the same folder deletes cleanly (`204`) once its document is trashed (`count_active` returns `0`) or removed.
...
- ~~Scope locks (4.7, "entire folder area locked for regular users") are not part of this session — conceptually they belong more to the Permission Service and are planned for a later phase.~~ — **closed since P3-S4**: `permission-service`'s `POST/DELETE /scope-locks` already applies to the entire subtree starting at a `resource_id`, and every folder gets a real `ResourceNode` via `folder.resource.created` — "entire folder area locked for regular users" is already fully achievable today via `POST /scope-locks` with a folder's `resource_id`. Stale bullet, the "later phase" it refers to already happened; found and struck during Phase 65+'s gap-analysis round.
...
- ~~**Deletion register not differentiated by backup** (5.2a) — unlike `document-service`, which closed this at P11-S4 (a separate, independently-published event feeding both `audit-service`'s hash chain and its own `GET /deletion-register`), `folder-service` never gained the equivalent fix; still genuinely open.~~ — **closed**: `audit-service`'s deletion ledger (`deletion_ledger.py`, `FORCE_DELETION_EVENT_TYPES`) already covers `folder.force_deleted`, not just `document.force_deleted` — an append-only file outside the shared Postgres instance, immune to `scripts/restore.sh`, exactly the backup-independence this bullet asks for. Combined with this service's own pre-existing `GET /deletion-register?folder_id=` (since P7-S1b), `folder-service` already has both pieces. `audit-service.md` itself already documents the mechanism as covering folders — this was a cross-file inconsistency, not a real gap; found and struck during Phase 65+'s gap-analysis round. ~~`audit-service` does not yet consume `folder.>` (a pre-existing gap, not introduced in this session), so this compensation is entirely missing here~~ — **closed in Phase 57 Session 1 (doc correction)**: `audit-service` has consumed `folder.>` since **P7-S2** (see `docs/services/audit-service.md`), so the hash-chain-only partial compensation `document-service` itself relied on before its own P11-S4 fix does apply here too — not "entirely missing," just not backup-differentiated on its own.
