...
|---|---|---|
| `POST` | `/cases` | Create (`name`, optional `object_type_id`/`attributes`, `process_definition_id`, `created_by`, optional `initial_data`) — validates `object_type_id` (if set) against the Object-Type Service (always as a root object, no folder parentage). Since **P15-S3**, also automatically assigns a `vorgangsnummer` (case reference number, 2.3/2.5), see below. Since **Post-Roadmap Phase 31 Session 2**: optional `draft` field skips that assignment, see "Draft / Pre-Registration Lifecycle" below. **Since P56-S1** ([ADR 0152](../adr/0152-maintenance-mode-service-to-service-enforcement-scoping.md) "Category A"): `503` while system-wide maintenance mode is active, checked before authentication. **Since P68-S1** ([ADR 0200](../adr/0200-p66s2-business-key-validation-regression-correction.md)): the `Case` row is now created and committed **before** the process instance is started in workflow-service (with `business_key = case_id`, `process_instance_id` filled in via a follow-up update once the instance exists) — previously the instance was started first, which briefly (P66-S2 only) broke case creation entirely once workflow-service started validating `business_key` against this service's own `GET /cases/{id}`, since the case genuinely didn't exist yet at that moment. `400` for an unknown `process_definition_id` — the already-committed row is removed again via a narrowly-scoped `repository.delete_unstarted_case` before returning, preserving "nothing persisted on a 400" |
...
- ~~**No retry for a failed records disposal** — a `failed` `CaseArchivalTransfer` in `archival-service` remains terminal; a repeated `POST /cases/{id}/archive-request` here would fail on the active-transfer exclusion there, as long as the old row is not handled separately.~~ — **closed**: `archival-service` already has `POST /case-archival-transfers/{id}/retry` (since ADR 0078, Post-Roadmap Phase 20 Session 2), the case counterpart to the document-level retry endpoint — resets a `failed_permanent` row back to `pending`, picked up by the existing poll loop automatically. This is exactly the "old row handled separately" case this bullet's own conditional clause anticipated. Found and struck during Phase 65+'s gap-analysis round.
