...
All endpoints except `/healthz` require `X-DMS-Principal` (401 without it) and the configured mail room role (403 without it). Since Post-Roadmap Phase 32 Session 3 ([ADR 0132](../adr/0132-mail-connector-department-rbac-group-membership.md)), acting on a `departmental` mailbox's content additionally requires `permission-service` group membership in that mailbox's `owning_group_id` — see "Department RBAC" below.
...
- **One poll loop, N mailboxes**: `_poll_loop` iterates every configured mailbox each tick (one
  ...
  `except` within the tick, so one mailbox's transient failure doesn't block the others. Since
...
- **Deliberately unchanged in this session**: outbound/SMTP (stays one global config; the plan's own
  wording is about *inbox* multiplicity). RBAC narrowing by department was deliberately deferred here —
  `poststelle_role` stayed one single, global role with every holder seeing every mailbox — and was
  picked up later in Post-Roadmap Phase 32 Session 3, see "Department RBAC" below.

## Department RBAC (14.2, Post-Roadmap Phase 32 Session 3, [ADR 0132](../adr/0132-mail-connector-department-rbac-group-membership.md))

`owning_group_id` (above) went from pure configuration metadata to an actually enforced gate: reading,
routing (source side only), confirming, manually assigning, or rejecting a message in a `departmental`
mailbox now additionally requires the caller to be a member of that mailbox's `owning_group_id` group in
`permission-service`. A `central` mailbox stays open to every `poststelle_role` holder, unchanged.

- **New `PermissionServiceClient.is_group_member(group_id, principal_id)`**: fetches `GET
  /groups/{group_id}/members` and filters client-side — no dedicated "is X a member of Y" endpoint exists
  at permission-service, and the first group-membership-as-RBAC-gate pattern in this codebase (every other
  `PermissionServiceClient` elsewhere only wraps capability/permission checks).
- **`_require_mailbox_access(mailbox_id, principal_id)`**: the single-mailbox gate, used by
  `GET /inbound/{id}`, `POST /inbound/{id}/route` (source mailbox only — the target stays unrestricted, so
  central intake can still route to any department), `confirm-match`, `assign`, and `reject`. An
  unconfigured `mailbox_id` is let through unchecked (same lenient behavior these endpoints already had for
  an unknown id before this session).
- **`_accessible_mailbox_ids(principal_id)`**: every `central` mailbox plus every `departmental` mailbox
  the caller is a member of — used to scope `GET /inbound`/`GET /routing-log` when called **without** an
  explicit `mailbox_id` (an explicit, inaccessible `mailbox_id` gets a hard `403` instead of being silently
  dropped from an unfiltered list).
- **`GET /mailboxes` stays deliberately unfiltered** — it backs the routing-target selector, which needs
  every mailbox's name visible so central intake can route to a department its own staff aren't members
  of; only mailbox *content* is now scoped, not the mailbox list itself.
- **`GET /routing-log`'s existing either-side hop matching still applies** — a hop touching an accessible
  mailbox on either end (most commonly `central`, always accessible) stays visible even to a non-member of
  the *other* end; true exclusion only happens for a hop touching no accessible mailbox on either side.
...
- **No topology restriction** — any configured mailbox may route to any other. Since P32-S3 ([ADR
  0132](../adr/0132-mail-connector-department-rbac-group-membership.md)), acting on the message's current
  (source) mailbox requires department-group membership if it's `departmental`; the *target* mailbox stays
  unrestricted either way, so central intake can still route to any department without needing membership
  there — see "Department RBAC" below.
...
2. Idempotency: every message carries a backend-specific, stable `source_uid` (POP3 UIDL) — already processed UIDs are skipped (`repository.get_by_source_uid`, scoped per mailbox since Post-Roadmap Phase 31 Session 12a — see "Multi-Inbox Model" above). **Since P62-S1**: this check is check-then-act, not atomic — only reachable with >1 replica polling the same mailbox concurrently (this service currently runs single-replica, so this is a defensive guard, not a currently-observed failure mode). `_ingest_message_or_skip_duplicate` (extracted from `_poll_loop` for testability) catches the resulting `IntegrityError` from `create_inbound_message`'s own `(mailbox_id, source_uid)` unique constraint and logs a benign no-op instead of letting it surface as an uncaught-looking stack trace. Investigated whether a losing replica could leave an orphaned storage object — it can't with this function's actual statement order: `create_inbound_message`'s `INSERT` is flushed, and its unique-constraint conflict raised, BEFORE any virus-scan/storage-upload call.
3. `_parse_message` breaks down the raw RFC-822 message (`email` standard library) into sender/subject/body/attachments. **Since P61-S4** ([ADR 0189](../adr/0189-webdav-edit-token-scope-and-mail-connector-size-limit.md)): `_ingest_message` checks `raw.raw_bytes`' length against `settings.max_message_size_bytes` (25 MiB default) BEFORE calling `_parse_message` at all — a message over the cap is never parsed (no MIME structure/base64 decode cost at all, previously unbounded and a real DoS vector against a service with genuine unauthenticated-adjacent external attack surface, any sender able to reach a configured mailbox); a minimal `InboundMessage` row is still created and immediately marked `status="rejected"` (`repository.mark_rejected`) so the mailbox's own idempotency check does not retry it every poll tick.
...
**Since P53-S4**: `_ingest_message` constructs its own short-lived `DocumentClient`/`CaseClient`/`VirusScanClient`/`StorageClient` and its own short-lived `NatsEventBusClient` (for the `mail_connector.message.received` publish), closing each in a `finally`, instead of reading the long-lived `app.state.documents`/`app.state.cases`/`app.state.virus_scan`/`app.state.storage`/`app.state.event_bus` instances — the same pattern `_load_candidate_pattern` already used for its own two clients (see that function's own docstring). This is the root-cause fix for this service's long-standing test flakiness (see "Tests" and "Open Points" below): `_ingest_message` is the one call site reached both from the background `_poll_loop` task (always on the lifespan/portal event loop) and directly from tests bypassing `TestClient`'s own request dispatch (a different event loop) — an `app.state.*` client is bound to whichever loop first uses it, so reusing one across both call paths intermittently broke, either as an httpx `RuntimeError: ... bound to a different event loop` or, for the NATS event bus specifically, an unexplained `nats.errors.TimeoutError` (a reply `Future` awaited from a different loop than the one whose background read-loop task resolves it never wakes up, so the wait just times out rather than raising immediately — found live while writing this session's own regression test). Every other endpoint in this file keeps reading `app.state.*` directly — they're only ever reached via one real HTTP request, always on the loop lifespan started on, so they don't have this problem.
...
- `uv run pytest services/mail-connector/tests` (**80 tests since P62-S1** — +1: `test_ingest_or_skip_duplicate_swallows_race_against_another_replica` (seeds a conflicting row directly, then confirms `_ingest_message_or_skip_duplicate` swallows the resulting `IntegrityError` instead of raising). Before that, 79 tests since P61-S4 — +1: `test_ingest_rejects_oversized_message_without_parsing_it` (`settings.max_message_size_bytes` monkeypatched low, asserts the message lands `status="rejected"` with no attachments, i.e. `_parse_message` was never called), see [ADR 0189](../adr/0189-webdav-edit-token-scope-and-mail-connector-size-limit.md). Before that, 78 tests since P53-S4 — +1: `test_two_back_to_back_ingests_do_not_hit_the_cross_event_loop_error`, the regression proof for the event-loop fix above, two real sequential `_ingest()` calls in one test that reliably reproduced the bug before the fix (5/5 in isolation) and passes reliably after it; the full suite's own run time additionally dropped from ~153s to a consistent ~20s, suggesting some of the intermittent slowness/hangs noted below in "Open Points" were this same root cause manifesting as delay rather than an outright failure, not only as the documented `RuntimeError`. Before P53-S4, 77 tests, previously 65 — since **Post-Roadmap Phase 32 Session 3** ([ADR 0132](../adr/0132-mail-connector-department-rbac-group-membership.md)): `test_api.py` (12 new: `GET /inbound`/`GET /inbound/{id}`/`POST /inbound/{id}/route` (source side)/`confirm-match`/`assign`/`reject`/`GET /routing-log` each 403 for a `poststelle_role` holder without department-group membership, the matching positive-membership success cases, an unfiltered `GET /inbound` correctly excluding an inaccessible departmental mailbox's messages while still showing `central`, routing to an inaccessible *target* department succeeding regardless (source-only enforcement), and a dedicated two-departmental-mailbox hop proving true either-side-inaccessible exclusion on `GET /routing-log`'s unfiltered listing, distinct from a hop merely touching the always-accessible `central` mailbox) — `with_finanzen_mailbox` extended to grant its test principal real `permission-service` group membership (via a new `_ensure_department_group` helper, find-or-create by name) instead of relying on the placeholder group id `"group-finanzen"`, which the newly enforced gate would otherwise reject outright — before that, 65 tests, previously 58 — since **Post-Roadmap Phase 31 Session 12c** ([ADR 0125](../adr/0125-postbuch-register-in-service-filtering-no-search-service.md)): `test_repository.py` (5 new: `search_routing_log` returns hops across messages newest-first, filters by `mailbox_id` on either side, filters by `routed_by`, filters by subject substring `q`, filters by `since`/`until` date range), `test_api.py` (2 new: auth gate on `GET /routing-log`, a hop returned with its message context incl. subject/current mailbox/status, `mailbox_id`/`q` filtering — the latter seeds its two messages directly via `repository.create_inbound_message` rather than two `_ingest` calls, same event-loop-flakiness-avoidance reasoning as the P31-S12b duplicate-collision test below) — before that, 58 tests, previously 47 — since **Post-Roadmap Phase 31 Session 12b** ([ADR 0124](../adr/0124-mail-routing-hop-log-orthogonal-to-matching.md)): `test_repository.py` (4 new: routing updates `mailbox_id` and logs the hop, multiple hops are each logged in order, an empty log for a never-routed message, `DuplicateInTargetMailboxError` when the target mailbox already holds the same `source_uid` — mailbox_id/routing log stay unchanged), `test_api.py` (7 new, via a `with_finanzen_mailbox` fixture monkeypatching `settings.mailboxes` with a second test mailbox: auth/role gate, `404` unknown message, `422` unconfigured target, `422` same-mailbox target, a full success round trip incl. the returned `routing_log` entry and the message disappearing from the origin mailbox's filtered listing, `409` on an already-confirmed message, `409` — not a raw `500` — when the target mailbox already has a colliding `source_uid`, found live during this session's own verification, see "Routing Between Mailboxes" above) — before that, 47 tests, previously 41 — since **Post-Roadmap Phase 31 Session 12a** ([ADR 0123](../adr/0123-multi-inbox-model-env-var-config-no-department-rbac-yet.md)): `test_repository.py` (2 new: `source_uid` scoped per mailbox — two different mailboxes may reuse the same UID without colliding; `list_messages` filters by `mailbox_id`), `test_api.py` (4 new: `GET /mailboxes` requires principal/role, returns the configured default mailbox with no credential fields present, `GET /inbound?mailbox_id=` filters correctly), plus the existing ingestion test extended with a `mailbox_id` assertion; `test_imap_backend.py`'s dedup-contract test updated for the new per-mailbox `get_by_source_uid` signature) — before that, 41 tests, previously 30: `matching.py` (candidate extraction, unambiguous/ambiguous/missing match via fake clients), repository (CRUD for inbound/outbound messages), API (role gate, full ingestion via `_ingest_message` directly instead of via a real POP3 connection — faster/more deterministic, candidate matching against real `document-service`/`case-service` incl. case fixture via the real `workflow-service`, confirm/assign/reject incl. 400/409, outbound send against real `mailpit` incl. file attachment from `related_document_id` — verified via mailpit's own REST API, filename/content-type/bytes of a real uploaded test document), IMAP backend (`test_imap_backend.py`, mocked at the `imaplib` boundary instead of running against a real server — see [ADR 0095](../adr/0095-imap-backend-mocked-imaplib.md) for the rationale: stable composite UID, no repeated deletion/marking on a repeated poll tick, real dedup contract via `repository.get_by_source_uid`, TLS/plain class selection) — the remaining part continues to run against real Postgres AND the real running sibling services, no mocks (same rationale as throughout the project).
- Live verified: real SMTP→POP3 roundtrip against `mailpit` within the running compose stack (see PROGRESS.md); **since P53-S4** additionally: a real email sent via SMTP to the running `mailpit` container was picked up by the real, rebuilt container's poll loop, scanned (`scan_status: "clean"`), staged, and appeared correctly in `GET /inbound` — confirming the refactored `_ingest_message` still works end-to-end in production's single-event-loop case (unaffected by this session's change, as expected, since production only ever calls it from the poll loop's own loop); since P24-S3 additionally a real SMTP→IMAP roundtrip against a temporary `greenmail` container as well as a real outbox attachment send (see PROGRESS.md). Since **Post-Roadmap Phase 31 Session 12a**: the ad-hoc migration verified live against the real dev stack's 653 pre-existing messages (all correctly backfilled to `mailbox_id="central"`, the old single-column unique constraint correctly replaced with the composite one), plus a genuine end-to-end SMTP→POP3 roundtrip through the real, rebuilt poll loop confirming a newly arriving message is correctly tagged with `mailbox_id` (see PROGRESS.md). Since **Post-Roadmap Phase 31 Session 12b**: a temporary second `finanzen` mailbox (same physical `mailpit` account as `central`, deliberately, to reproduce the collision) confirmed `DuplicateInTargetMailboxError` surfaces as a clean `409` against the real, rebuilt container rather than a raw `500` (see PROGRESS.md). Since **Post-Roadmap Phase 31 Session 12c**: a temporary second `finanzen` mailbox with a deliberately unreachable POP3 host (avoiding P31-S12b's shared-account duplication artifact entirely) confirmed a genuine successful routing hop against the real, rebuilt container, and `GET /routing-log` correctly surfaced it with working `mailbox_id`/`q` filters (see PROGRESS.md). Since **Post-Roadmap Phase 32 Session 3**: a real `permission-service` group and membership were created against the running stack, and `GET /mailboxes`/`GET /inbound`/`GET /routing-log`/`/healthz` confirmed no regression against this installation's current single-mailbox (`central`-only) live configuration — the departmental gate itself has no live department to exercise yet in this installation (`DMS_MAILBOXES` still configures only `central`), so its enforcement is otherwise covered by the test suite's own real-`permission-service` calls (see PROGRESS.md).
...
- ~~**Some subset of the tests that call `_ingest()` directly (bypassing `client.post`) intermittently fail on a full-suite run** with `RuntimeError: ... bound to a different event loop`, traced to `app.state.virus_scan`'s long-lived async client being reused across pytest-asyncio test functions that don't share an event loop~~ — **fixed at the root in P53-S4**: `_ingest_message` now constructs every client it uses (`DocumentClient`/`CaseClient`/`VirusScanClient`/`StorageClient`, plus its own `NatsEventBusClient` for the completion event) fresh per call instead of reading the long-lived `app.state.*` instances, closing them in a `finally` — see "Ingestion pipeline" above. This was re-confirmed three times without a root fix (Post-Roadmap Phase 31 Session 12, Phase 38 Session 4, Phase 44 Session 3, each time noted as "would need restructuring `app.state.virus_scan`'s client lifecycle, out of scope for that session"); this session was explicitly that restructuring. Diagnosing it precisely also surfaced a second, previously-unknown instance of the identical root cause on `app.state.event_bus` (NATS) — manifesting not as a `RuntimeError` but as an unexplained `nats.errors.TimeoutError` (a reply `Future` awaited from a different loop than the one whose background read-loop task resolves it never wakes up, so the wait just times out) — found live while writing this session's own regression test (`test_two_back_to_back_ingests_do_not_hit_the_cross_event_loop_error`, two real, sequential `_ingest()` calls in one test, previously documented elsewhere in this file as reliably flaky and deliberately avoided).
- ~~**Only POP3 implemented**~~ — **IMAP built since P24-S3** (`backends/imap_backend.py`, `ImapBackend`). **Microsoft Graph (Exchange/O365) remains deliberately open, now with a concrete build recommendation from Post-Roadmap Phase 43 Session 3** ([ADR 0161](../adr/0161-mail-connector-graph-backend-scoping.md), scoping only, no code changed): a fourth `MailboxBackend` implementation, same interface, no structural change — OAuth2 client-credentials with application permissions (not delegated, no signed-in user for a headless poller), fetching raw MIME directly via Graph's `GET /messages/{id}/$value` endpoint (sidesteps any JSON-to-MIME reconstruction, so `main.py`'s existing parsing pipeline needs zero changes), `$filter=receivedDateTime gt ...` polling on the existing tick interval rather than delta queries/webhooks (avoids new persisted state this schema has never needed), a client secret rather than a certificate credential (simpler, reference-implementation-appropriate, same judgment call as `archival-service`'s single static encryption key), and the existing `libs/dms-retry` backoff idiom for Graph's own rate limiting. Flagged as this project's first-ever external OAuth2 client-credentials integration (no existing precedent to reuse — Keycloak tokens are internal, `federation-hub-service`'s trust model is RSA signing, not bearer tokens) — the one genuinely novel piece of engineering a build session should budget real time for, even though it slots cleanly into the existing backend interface otherwise. `backends/interface.py`'s `MailboxBackend` interface itself needed no changes to support this scoping's recommendations - confirming ADR 0095's own original anticipation held up three sessions later.
...
- ~~**Only one mailbox per installation**~~ — **fixed in Post-Roadmap Phase 31 Session 12a**: any number of named mailboxes (see "Multi-Inbox Model" above). ~~**No frontend mailbox selector**~~ — **fixed in Post-Roadmap Phase 31 Session 12b**: `PoststellePane.tsx` gained a filter + routing UI (see "Routing Between Mailboxes" above), shown once more than one mailbox is configured. ~~**No searchable, standalone cross-mailbox "Postbuch" register view**~~ — **fixed in Post-Roadmap Phase 31 Session 12c**: `GET /routing-log` + a third `PoststellePane.tsx` tab (see "Postbuch Register" above). ~~**No per-mailbox/department RBAC**~~ — **fixed in Post-Roadmap Phase 32 Session 3** ([ADR 0132](../adr/0132-mail-connector-department-rbac-group-membership.md)): departmental mailboxes now require real `permission-service` group membership (see "Department RBAC" above); `poststelle_role` still gates the service as a whole. **Still open, deliberately deferred to a later session**: admin-UI mailbox management (adding/reconfiguring a mailbox still needs an env var change + restart, consistent with `storage-service`/`signature-service`'s connector *lists* being equally restart-only, ADR 0091), and cursor-based pagination for the Postbuch register beyond its `limit` cap (ADR 0125 "Consequences").
