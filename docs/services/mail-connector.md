...
- **Two specific tests (`test_confirm_match_creates_document_in_matched_folder`, `test_route_already_confirmed_message_returns_409`) deterministically fail on a full-suite run** with `RuntimeError: ... bound to a different event loop`, traced to `app.state.virus_scan`'s long-lived async client being reused across pytest-asyncio test functions that don't share an event loop (found at larger scale during Post-Roadmap Phase 38 Session 4's own full backend-suite run, ADR 0149 - unrelated to that session's RBAC work). Both pass individually; the same class of issue was already worked around in specific spots at P31-S12b/c (see "Tests" above) but not universally. Not fixed in this session (would mean restructuring this file's `app.state` client lifecycle across the whole suite, out of RBAC scope).
