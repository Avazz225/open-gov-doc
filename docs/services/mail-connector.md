...
- **Some subset of the tests that call `_ingest()` directly (bypassing `client.post`) intermittently fail on a full-suite run** with `RuntimeError: ... bound to a different event loop`, traced to `app.state.virus_scan`'s long-lived async client being reused across pytest-asyncio test functions that don't share an event loop (found at larger scale during Post-Roadmap Phase 38 Session 4's own full backend-suite run, ADR 0149 - unrelated to that session's RBAC work). Re-confirmed in Phase 44 Session 3's own test runs (unrelated maintenance-mode work) - which specific test(s) fail varies non-deterministically run to run (observed variously: `test_ingest_detects_unique_kennzeichen_match`, `test_ingest_detects_unique_vorgangsnummer_match`, `test_confirm_match_creates_document_in_matched_folder`, `test_route_already_confirmed_message_returns_409`), confirming this isn't localized to two specific tests but to the whole class of tests using `_ingest()` directly. All pass individually; the same class of issue was already worked around in specific spots at P31-S12b/c (see "Tests" above) and, in Phase 44 Session 3, in `reporting-service`'s own equivalent case (a one-off drive-by fix, see `docs/services/reporting-service.md`) - but not universally here, since fixing it properly for this file would mean restructuring `app.state.virus_scan`'s client lifecycle across the whole suite, a larger, still out-of-scope change for either session.
