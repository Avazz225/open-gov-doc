...
| `GET` | `/documents/deleted?folder_id=...` | Gelöschte (im Papierkorb befindliche) Dokumente eines Ordners (5.2, seit P7-S1) — vor `/documents/{id}` registriert, damit `"deleted"` nicht als `{id}` interpretiert wird. Alternativ `?scope=personal\|admin\|admin_classified` statt `folder_id` (installationsweiter Papierkorb, 2.5, seit P15-S1), siehe "Papierkorb-Familie" unten |
| `POST` | `/documents/{id}/purge` | Manuelle, sofortige endgültige Löschung eines bereits im Papierkorb liegenden Dokuments (2.5, seit P15-S1) — `409` wenn nicht im Papierkorb, `403` ohne die passende Löschadministration-Rolle, siehe "Papierkorb-Familie" unten |
...
| `POST` | `/documents/{id}/reconcile-restore-deletion` | Löschabgleich nach Restore (10.4, seit P11-S4) — `X-DMS-Roles: dms-admin`, siehe unten |
...
`folder_id`/`object_type_id` sind opake Referenzen ohne FK-Erzwingung über Service-Grenzen hinweg, werden aber seit P3-S3 aktiv geprüft: `folder_id` (falls gesetzt) muss beim Folder Service existieren (sonst 400), `object_type_id` (falls gesetzt) validiert `attributes`+`title` gegen den Object-Type Service (sonst 400 mit Fehlerliste). Seit P4-S4 gilt dieselbe Validierung auch für `PATCH /documents/{id}`; `object_type_id` selbst bleibt dabei unveränderlich (Retypisieren ist eine eigene Operation mit anderen Konsistenzfragen). **`folder_id` ist seit P12-S1 die Ausnahme**: `DocumentUpdate.folder_id` (optional) verschiebt das Dokument in einen anderen Ordner, wenn gesetzt und vom aktuellen `folder_id` abweichend — Ziel-Existenzprüfung wie bei der Anlage (400 bei unbekanntem Zielordner), die Objekt-Hierarchie-Validierung (s. u.) wird dabei erneut gegen den *neuen* Elternordner durchgeführt. Grund: WebDAV MOVE (`webdav-connector`, siehe `docs/services/webdav-connector.md`) braucht zwingend eine Verschiebe-Operation für Dokumente — `folder-service`s `PATCH /folders/{id}` unterstützt `parent_id`-Änderungen bereits seit P3-S3, dieses Gegenstück für Dokumente fehlte bis dahin komplett.

**Erzwungene Objekt-Hierarchie (2.2a, seit P5b-S1, ADR 0013)**: Die Existenzprüfung des Zielordners (`FolderClient.get()`, ersetzt das frühere reine `exists()`) liefert seither den vollen Ordner-Body inkl. dessen `object_type_id` — dieser wird zusammen mit `parent_is_root` (`true`, wenn `folder_id` fehlt oder `"root"` ist) an `POST /object-types/{id}/validate` übergeben, damit ein `allowedParentTypes` des Dokument-Objekttyps durchgesetzt werden kann. Bei der Anlage (`POST /documents`) immer relevant; bei `PATCH /documents/{id}` nur, wenn `folder_id` tatsächlich einen Move auslöst (s. o.) — ein reines Metadaten-Update ohne `folder_id`-Änderung validiert die Hierarchie nicht erneut.
...
- **Zwei Sibling-Services checken automatisiert neue Versionen ein, wenn ihre Verarbeitung die Bytes tatsächlich verändert** (ADR 0025-Muster): `signature-service` nach dem Signieren einer PDF, und seit einem OCR-Nutzer-Feedback-Durchgang zusätzlich `ocr-service` nach Einbettung eines unsichtbaren Textlayers in einen zuvor textlayer-losen Scan (`created_by="system:ocr-service"`, siehe `docs/services/ocr-service.md` "Durchsuchbare PDF"). Beide nutzen exakt denselben `expected_base_version_number`-Mechanismus wie jeder andere Check-in — kein Sonderpfad, keine eigene Konfliktbehandlung nötig.
...
- **Löschregister nicht hash-verkettet** (anders als `audit-service`), **seit P11-S4 aber Backup-differenziert**: jede Zeile wird zusätzlich als reguläres Event publiziert, das sowohl `audit-service`s hash-verkettete Kette **als auch** dessen separates, vom DB-Restore-Zyklus unabhängiges Löschregister-Ledger mitschreibt (siehe `docs/services/audit-service.md`, `docs/operations/backup-restore.md`). Bewusst eine eigene, sofort abfragbare `GET /deletion-register`-API in diesem Service statt einer Abhängigkeit von einer `audit-service`-Filter-API.
- **Löschabgleich nach Restore (10.4, P11-S4)**: `POST /documents/{id}/reconcile-restore-deletion` (Gate `X-DMS-Roles` enthält `dms-admin`) führt `execute_forced_deletion` erneut aus — für den Fall, dass ein Restore ein bereits zwangsgelöschtes Dokument unbeabsichtigt wiederhergestellt hat. Wird von `scripts/restore.sh`s Erkennungsschritt referenziert, nicht automatisch von dort aufgerufen (siehe `docs/operations/backup-restore.md`).
...
- **Genehmigungs-Inbox**: neue, minimale `ApprovalsPane` in der User-UI (nicht Admin-UI, da sich reguläre Nutzer hier gegenseitig genehmigen) — siehe `docs/services/user-ui.md`. Bewusst nur für `document.delete`/`folder.delete` gefiltert, keine generische Alle-Aktionstypen-Inbox.

## Papierkorb-Familie: persönlicher & Verschlusssachen-Papierkorb (2.5, seit P15-S1)

Vollständige Umsetzung von Konzept §2.5's Papierkorb-Sonderbereichen (regulärer/persönlicher/Verschlusssachen-Papierkorb) auf Basis der bereits bestehenden Aufbewahrungs-/Papierkorb-Infrastruktur oben. Siehe [ADR 0051](../adr/0051-papierkorb-familie-classification-via-object-type-scoped-global-endpoints.md) für die vollständige Architekturbegründung.

- **`deleted_by`-Spalte nachgerüstet**: `repository.delete_document`/`cascade_trash_by_folder_ids` nahmen `deleted_by` bereits als Parameter entgegen, persistierten ihn aber nie (echte, bei P15-S0 gefundene Lücke) — jetzt eine echte `Document.deleted_by`-Spalte, Voraussetzung für den persönlichen Papierkorb. Wird bei `restore_document`/`cascade_restore_by_via_folder_id` wieder auf `None` zurückgesetzt.
- **Neuer `scope`-Query-Parameter auf `GET /documents/deleted`** (`personal`/`admin`/`admin_classified`) — rein additiv, ohne `scope` bleibt das bisherige, ordnerbezogene, ungegatete Verhalten unverändert (bestehende Aufrufer/Tests unberührt). Mit `scope=personal` sieht jeder authentifizierte Aufrufer ausschließlich `deleted_by == X-DMS-Principal` (401 ohne Principal-Header). Mit `scope=admin`/`scope=admin_classified` wird die jeweilige Rolle (`trash_hard_delete_admin_role`/`classified_trash_hard_delete_admin_role`, Settings, Default beide `"dms-admin"`) geprüft (403 sonst) — die Verschlusssachen-Menge wird über `object_type_client.list_classified_document_type_ids()` (neuer `ObjectTypeClient`-Methode, ruft `GET /object-types?applies_to=document&is_classified=true` am `object-type-service` ab) einmal pro Aufruf aufgelöst, keine N+1-Abfrage je Dokument.
- **Neuer `POST /documents/{id}/purge`**: manuelle, sofortige endgültige Löschung eines bereits im Papierkorb liegenden Dokuments (409 sonst), unabhängig vom automatischen `_retention_poll_loop`. Ruft dieselbe `retention_actions.purge_expired_trash_entry()` auf wie der Poll-Loop, jetzt um optionale `trigger`/`triggered_by`-Parameter erweitert (`trigger="manual_purge"` statt `"trash_expiry"`, `triggered_by` = echter `X-DMS-Principal`) — keine zweite Lösch-Implementierung. Rollenprüfung identisch zur `scope`-Auflösung oben (klassifiziert vs. regulär, je nach `ObjectType.is_classified` des betroffenen Dokuments).
- **`DeletionRegisterEntry.trigger`** um den Wert `"manual_purge"` erweitert (Pydantic-`Literal` + Modell-Kommentar aktualisiert).
- **Live gefundener Schema-Drift behoben**: `object_type_id` trug in diesem langlebigen Dev-Stack noch `VARCHAR(128)` (Überbleibsel einer frühen Projektphase, `create_all` ändert nie den Typ bestehender Spalten) statt des im Model längst deklarierten `Integer` — unauffällig bei Gleichheitsvergleichen, aber der neue `NOT IN`-Filter für den Verschlusssachen-Papierkorb deckte es live auf (`operator does not exist: character varying <> integer`). Neue, geschützte Migrationsprüfung im Lifespan (`information_schema.columns`-Check vor `ALTER COLUMN ... TYPE INTEGER`) korrigiert das automatisch bei jedem betroffenen Neustart, siehe [ADR 0051](../adr/0051-papierkorb-familie-classification-via-object-type-scoped-global-endpoints.md) "Live-Verifikationsfund".
