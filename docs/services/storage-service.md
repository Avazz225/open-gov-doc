...
- `uv run pytest services/storage-service/tests` (**76 Tests**, davon 22 neu seit P5b-S6): Backend-Plugins (`LocalFilesystemBackend` gegen `tmp_path`, `S3Backend` gegen echtes MinIO), `backends/__init__.py`-Fabrikfunktionen (neu: mehrere gleichartige Instanzen im selben Ziel-Set, `BackendTargetConfig`-Pflichtfeldvalidierung je Typ), Repository (Metadaten/Kopien wie zuvor, neu: `backend_identity`/`guard_config`-CRUD, `reset_copies_for_backend`, `count_pending_copies_by_backend`), `identity_guard.check_target_identity` (neu: Erststart/Bootstrap, bestätigter Match, fehlende Marker-Datei nach bekannter Identität, abweichende Marker-Datei, nicht erreichbares Backend mit/ohne bekannte Identität — alle gegen eine echte `LocalFilesystemBackend`/eine reale, aber immer scheiternde Testdouble-Implementierung, kein Mocking), Replikation (unverändert, backend-agnostisch getestet mit frei gewählten IDs), API (`/guard-config`, `/guard-status` neu, `/healthz` zeigt jetzt `primary_target`/`targets` statt `backend`).
- **Live-Verifikation ohne Mocking**: ein echter Datenträger-Wechsel wurde gegen den laufenden Container simuliert (Identitätsdatei manuell verändert, Neustart erzwungen) — Startverweigerung, Admin-Override + degradierter Start, automatische Nachreplikations-Vormerkung und `POST /replication/process-pending` alle 1:1 wie vorgesehen; siehe `PROGRESS.md` für den Ablauf.

## Offene Punkte

- **Rebalancing** beim Hinzufügen/Entfernen eines Ziels aus einem laufenden Ziel-Set — ein neu hinzugefügtes Ziel wird zwar automatisch identitäts-geprägt, bekommt aber keine automatische Kopie bereits bestehender Objekte. Konzeptionell mit dem Migrationsprozess aus 7.2 verwandt, eigene spätere Session (unverändert seit ADR 0004).
...
- **Keine automatische periodische Ausführung** von `/object-verify/.../all` oder `/replication/process-pending` — beide sind bewusst reine On-Demand-Endpunkte, gedacht für einen externen Scheduler (siehe ADR 0004), noch nicht Teil dieser Session. Nach einem degradierten Start bleibt die Wiederherstellung deshalb so lange "hängen" (`pending_copies > 0`), bis jemand/etwas den Endpunkt aufruft.
- **Kein Korrekturmechanismus für `backend_identity`** — ein beabsichtigter, legitimer Datenträgertausch (z. B. geplanter Hardware-Wechsel) muss den Referenzwert aktuell direkt in der Tabelle korrigieren lassen; kein Admin-UI-Workflow dafür (siehe ADR 0017 "Konsequenzen").
