...
- **Keine automatische periodische Ausführung** von `/object-verify/.../all` oder `/replication/process-pending` — beide sind bewusst reine On-Demand-Endpunkte, gedacht für einen externen Scheduler (siehe ADR 0004). Nach einem degradierten Start oder einem `reidentify`-Aufruf bleibt die Wiederherstellung deshalb so lange "hängen" (`pending_copies > 0`), bis jemand/etwas den Endpunkt aufruft. **Seit Post-Roadmap Phase 20 Session 6** ([ADR 0082](../adr/0082-storage-service-replication-jitter-retrofit.md)) hat die Retry-Queue selbst bereits Full-Jitter-Backoff — es fehlt nur noch der externe Träger dafür, geplant als k8s-`CronJob`-Template im neuen Helm-Chart (**P26-S4**).
