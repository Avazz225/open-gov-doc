...
**Object-CRUD data-plane authorization (Phase 59 Session 2, [ADR 0179](../adr/0179-storage-service-object-crud-trusted-caller-gate.md))**: all eleven object-CRUD endpoints (upload/download/delete, metadata, copies, both fixity-check endpoints, and the three archive-copy endpoints — see the API table above) previously had **no permission check of any kind**, unauthenticated by any measure — a real, unexploited-in-practice-but-genuinely-exploitable bypass found by this round's live-code security sweep, letting any authenticated user through the gateway read/write/delete any object by key, completely bypassing `document-service`'s per-document ACL. Now requires `X-DMS-Principal` to be one of six known trusted internal callers (`document-service`, `archival-service`, `rendering-service`, `ocr-service`, `virus-scan-service`, `mail-connector`) — `403` otherwise, including for a missing header. Deliberately NOT the existing `admin.storage` capability (a human-administrator concern, wrong semantic for six backend services doing their normal job on an already-authorized end user's behalf, see ADR 0179's own rationale) — a fixed-caller-set check instead, the same "trusted system identity" convention this project uses for its single-caller precedents, extended to accept several literal identities. Each of the six calling services' own `StorageClient`/equivalent now sends its fixed identity as a default header on every request, confirmed unspoofable for a real end user the same way as those precedents (the gateway always overwrites any client-supplied `X-DMS-Principal` with the verified JWT claims). **Closed (P66-S1, [ADR 0194](../adr/0194-p66s1-small-security-fixes-bundle.md))**: `GET /storage/usage` and the two `/process-pending` maintenance endpoints, previously left ungated as a materially lower-risk aggregate-statistics/internal-maintenance surface, now require the same `_require_storage_caller` gate. Two new trusted callers were added to the allowlist for this: `reporting-service` (needs `GET /storage/usage` for its own usage report — its `StorageClient` was also found, in the same session, to have never sent an `X-DMS-Principal` header at all on any call, an independent, already-broken production bug, fixed alongside) and `system:storage-replication-cronjob` (the identity the scheduled `CronJob` uses to trigger the two maintenance endpoints). **P67-S2** ([ADR 0198](../adr/0198-cli-migration-license-plugin-orchestration-deletion-register-commands.md)): a real regression from the P66-S1 gate above was found and fixed — `license-service`'s `StorageClient.total_bytes()` (the `storage_gb` license-usage dimension, a real caller of `GET /storage/usage` since P9-S0) was missed when the allowlist was first built, breaking `GET /license/status` (`500`, found via this session's own live CLI verification). `license-service` added to the allowlist; its `StorageClient` now sends `X-DMS-Principal: license-service`.
...
**Since P71-S1** ([ADR 0205](../adr/0205-p71s1-notification-wiring-bundle.md)): this service's first ever event bus connection, a new `"storage"` stream (`ensure_stream=True`). Previously "neither publish nor consume events" — alerting-worthy failures were "logged instead of alerted" (see "Redundancy & Fixity" above). Consumed by nothing; published only, both picked up by `notification-service` (own durable names there, see `docs/services/notification-service.md`):

| event_type | payload | trigger |
|---|---|---|
| `storage.replication.failed_permanent` | `{object_key, backend_id, attempts, error}` | `replication.process_pending` - a catch-up copy exhausted `max_replication_attempts` |
| `storage.object_verify.mismatch` | `{object_key, backend_ids}` | `replication.verify_pending` - the bulk fixity sweep found a checksum mismatch on at least one backend |

`publish_event` is threaded through both functions as an optional parameter (`None` in tests/call sites that don't care) - neither function's core retry/verification logic depends on it.
...
- ~~**`upload_object`/`upload_archive_copy` read the entire request body into memory with no size cap anywhere**~~ — **closed in Phase 61 Session 2** ([ADR 0187](../adr/0187-shared-max-upload-size-middleware.md)): `dms_common.MaxBodySizeMiddleware`, `413` above `max_upload_size_bytes` (200 MiB default), applied globally to every route. Same fix shared with `virus-scan-service`/`rendering-service`.
