...
`GET /events`/`GET /events/verify` previously had **no role check at all** — anyone with network access to the gateway could read the complete audit trail. Both now require `X-DMS-Principal` + the new capability `audit.read` (401 without the header, 403 without the permission). Added to the "everyone" group (any authenticated principal) rather than a dedicated domain-admin role: several existing internal callers (`query-service`'s `_run_query`, `reporting-service`'s forensic trace/scheduled reports) had zero auth before, and each now forwards the ALREADY-authenticated calling principal whose own outer endpoint gate already ran, rather than requiring every caller to be individually re-granted. `permission-service`'s `EVERYONE_ROLE_PERMISSIONS` is not self-healing — an already-running installation's existing "everyone" role needs the permission added once via `PUT /roles/{id}`, same as any other addition to that list.

## Tests

- `uv run pytest services/audit-service/tests`: basic hash chain functions (`test_hashchain.py`, unchanged), repository including new cutover tests (timestamp computation, idempotency, verification across the cutover boundary with entries manually constructed as "legacy rows") and filter combinations (`actor`/`subject`/`event_type` exact+wildcard/time window), consumer integration test against real NATS including `actor` roundtrip. **37 tests** (previously 31, +6, since **Post-Roadmap Phase 38 Session 2**: new `test_api.py` — this service had no HTTP-API-level test file before this session — covering `GET /events`/`.../verify` each for the 401/403/200 cases against the new `audit.read` gate).
