...
**Since Post-Roadmap Phase 75 Session 6** ([ADR 0223](../adr/0223-reviewer-ui-tailwind-rollout-cases-pane-dead-css-fix.md)): the rest of this app's UI (`Shell`, `ThemeSwitcher`, `LocaleSwitcher`, `MaintenanceBanner`, `RequireAuth`, `TaskList`, `ApprovalList`, `TeamTaskList`, `InstanceDetail`, `CasesPane`) converted from hand-written CSS classes to Tailwind utilities — completing the small-apps rollout group. Found and fixed a real, pre-existing bug as a byproduct: `CasesPane.tsx` referenced three CSS classes (`.cases-pane`/`.pane-heading`/`.entry-name`/`.entry-meta`) that were never actually defined anywhere in this app's CSS (confirmed via grep) — this case-browsing UI has been rendering completely unstyled since it was added in Post-Roadmap Phase 74 Session 1. Given real styling now, matching the rest of this app's Tailwind visual language.
