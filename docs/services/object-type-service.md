...
|---|---|---|
| `POST` | `/object-types` | Anlegen (`name`, `applies_to`, `attributes`, `naming_constraints`, `conditions`, `allowed_parent_types`, `icon`, `required_signature_level`, `default_retention_days`, `deletion_reason_required_override`, `default_archive_after_days`, `archive_encryption_enabled`, `classification_level`) — 409 bei doppeltem Namen, 422 bei ungültigem `allowed_parent_types`/`icon`/`required_signature_level`/`default_retention_days`/`default_archive_after_days`/`classification_level` (siehe 2.2a bzw. "Mindest-Signaturniveau"/"Aufbewahrung"/"Aussonderung"/"Verschlusssachen-Einstufung" unten) |
| `GET` | `/object-types?applies_to=document\|folder` | Liste, optional gefiltert |
| `GET` | `/object-types/{id}` | Einzelne Definition |
| `PUT` | `/object-types/{id}` | Definition ersetzen (Attribute/Naming/Conditions/`allowed_parent_types`/`icon`/`required_signature_level`/`default_retention_days`/`deletion_reason_required_override`/`default_archive_after_days`/`archive_encryption_enabled`/`classification_level`) — `name`/`applies_to` bleiben unveränderlich |
...
| `POST` | `/object-types/{id}/next-kennzeichen` | Kennzeichengenerator (2.2, seit P5e-S1): atomarer Inkrement+Format-Aufruf, liefert `{kennzeichen: "2026-001"}` — 404 bei unbekannter `object_type_id` oder wenn kein `kennzeichen_format` konfiguriert ist. **Seit P17-S2** optionaler Body `{attributes: {...}}` für attributbasierte Platzhalter (14.2, z. B. `{Federführung}`) — 422, wenn ein referenzierter Platzhalter keinen Wert erhält |
...
- **Unterstützte Platzhalter**: `{YYYY}` (vierstelliges Jahr), `{YY}` (zweistellig), `{MM}`/`{DD}` (zweistellig), `{Laufende_Nummer}` (dreistellig nullgepolstert, z. B. `001`, wächst darüber hinaus einfach in der Ziffernzahl). `kennzeichen_format` muss `{Laufende_Nummer}` enthalten. **Seit P17-S2** (14.2): jeder weitere Platzhalter, der keiner der obigen Datums-/Zähler-Platzhalter ist, wird als Name eines am Objekttyp definierten Attributs interpretiert (z. B. `{Federführung}`) — muss beim Speichern des Formats tatsächlich als Attributname existieren, sonst `422` (analog zur `allowedParentTypes`-Referenzprüfung, 2.2a). `POST .../next-kennzeichen` erwartet die Attributwerte dann im optionalen Body `{attributes: {...}}` (von `document-service` beim Dokumentanlegen mitgeschickt) — fehlt ein referenzierter Wert trotzdem (Attribut nicht als Pflichtfeld markiert), liefert der Endpunkt `422` statt eine Kennzeichnung mit stillschweigend leerer Lücke zu erzeugen. Direkte Umsetzung des Konzeptbeispiels `{Abteilung}-{YYYY}-{Laufende_Nummer}` (14.2), siehe [ADR 0059](../adr/0059-egov-paket-aktenplan-hierarchie-und-mehrstufige-vs-einstufung.md) für die Begründung, warum das eGov-Paket konkret `{Federführung}` statt `{Abteilung}` nutzt.
...
## Verschlusssachen-Einstufung (2.5, seit P15-S1, mehrstufig seit P17-S2/14.2)

`classification_level` (String, nullable) — nur für `applies_to="document"` zulässig (422 sonst, gleiches Validierungsmuster wie `kennzeichen_format`/`required_signature_level`), einer der vier gängigen deutschen VS-Einstufungen (`VS-NfD`/`VS-VERTRAULICH`/`GEHEIM`/`STRENG GEHEIM`, wörtlich aus dem Konzepttext 14.2) oder `null` = nicht eingestuft. **Bis P17-S1 ein reines Bool** (`is_classified`) — jeder gesetzte Wert (unabhängig von der konkreten Stufe) löst weiterhin denselben Gate aus, die Stufe selbst ist reine Zusatzinformation, siehe [ADR 0059](../adr/0059-egov-paket-aktenplan-hierarchie-und-mehrstufige-vs-einstufung.md). `document-service` löst darüber den strukturell getrennten Verschlusssachen-Papierkorb auf (siehe `docs/services/document-service.md` "Papierkorb-Familie") — `GET /object-types?applies_to=document&is_classified=true` (Parametername unverändert, siehe unten) liefert die vollständige Menge eingestufter Objekttyp-IDs in einem Aufruf, statt pro Dokument einzeln nachzuschlagen. [ADR 0051](../adr/0051-papierkorb-familie-classification-via-object-type-scoped-global-endpoints.md) begründet die ursprüngliche Entscheidung für eine schema-gebundene statt attribut-wert-basierten Kennzeichnung — an diesem Muster ändert P17-S2 nichts, nur der Werteraum wurde erweitert. Der `GET /object-types`-Filter-Parameter `is_classified: bool` bleibt trotzdem so benannt (nicht `classification_level`) — Aufrufer fragen "irgendeine Einstufung oder keine", nie eine konkrete Stufe; intern übersetzt auf `classification_level IS (NOT) NULL`.
