...
|---|---|---|
| `POST` | `/object-types` | Anlegen (`name`, `applies_to`, `attributes`, `naming_constraints`, `conditions`, `allowed_parent_types`, `icon`, `required_signature_level`, `default_retention_days`, `deletion_reason_required_override`, `default_archive_after_days`, `archive_encryption_enabled`, `is_classified`) — 409 bei doppeltem Namen, 422 bei ungültigem `allowed_parent_types`/`icon`/`required_signature_level`/`default_retention_days`/`default_archive_after_days`/`is_classified` (siehe 2.2a bzw. "Mindest-Signaturniveau"/"Aufbewahrung"/"Aussonderung"/"Verschlusssachen-Kennzeichnung" unten) |
| `GET` | `/object-types?applies_to=document\|folder` | Liste, optional gefiltert |
| `GET` | `/object-types/{id}` | Einzelne Definition |
| `PUT` | `/object-types/{id}` | Definition ersetzen (Attribute/Naming/Conditions/`allowed_parent_types`/`icon`/`required_signature_level`/`default_retention_days`/`deletion_reason_required_override`/`default_archive_after_days`/`archive_encryption_enabled`/`is_classified`) — `name`/`applies_to` bleiben unveränderlich |
...
## Verschlusssachen-Kennzeichnung (2.5, seit P15-S1)

`is_classified` (Boolean, Default `false`) — nur für `applies_to="document"` zulässig (422 sonst, gleiches Validierungsmuster wie `kennzeichen_format`/`required_signature_level`), keine eigene Klassifizierungsstufen-Systematik (rein binär, Konzepttext ausdrücklich). `document-service` löst darüber den strukturell getrennten Verschlusssachen-Papierkorb auf (siehe `docs/services/document-service.md` "Papierkorb-Familie") — `GET /object-types?applies_to=document&is_classified=true` liefert die vollständige Menge klassifizierter Objekttyp-IDs in einem Aufruf, statt pro Dokument einzeln nachzuschlagen. [ADR 0051](../adr/0051-papierkorb-familie-classification-via-object-type-scoped-global-endpoints.md) begründet die Entscheidung für eine schema-gebundene statt attribut-wert-basierten Kennzeichnung.
