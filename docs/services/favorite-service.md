...
|---|---|---|
| `POST` | `/favorites` | Create (`user_id`, `object_type`: `"document"`\|`"folder"`\|`"case"`, `object_id`) — `201`, `409` if the favorite already exists (unique constraint `user_id`+`object_type`+`object_id`) |
