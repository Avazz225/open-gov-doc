...
- ~~**10.1 sensor infrastructure only exists from Phase 11 onward** (P10-S0 finding) — the service's own snapshot here is a deliberately minimal transitional solution, not an anticipation of the full-fledged monitoring layer.~~ — **the infrastructure itself has existed since Phase 11; this service's own sensors were added additively in Phase 40 Session 4** (see "Sensors" below). The `psutil`-sampled snapshot itself (`cluster_node`) remains unchanged and is NOT replaced by the sensors — `placement.py`'s scheduling algorithm needs a directly queryable DB row, which a write-only `GuardedGauge` cannot provide.
...
Two gauges, `plugin_orchestration.node.cpu_usage_percent`/`.available_ram_mb` (group `capacity`), migrating this service's `psutil`-sampled node data onto the real sensor infrastructure (previously only the generic `http.*` sensors existed here). **Additive, not a replacement**: `sampler.run_tick` sets both gauges from the SAME sampled values dict it already computes for the `cluster_node` upsert (no second, independent `psutil` call) — `cluster_node` itself is unchanged and remains the row `placement.py`'s scheduling algorithm reads directly; a `GuardedGauge` has no getter (only externally readable via a `/metrics` scrape) and could never serve that purpose. `sampler_loop`/`run_tick` both take the two gauges as optional keyword parameters (default `None`) for backward compatibility with existing direct callers/tests that don't pass any.
...
- ~~The service's own resource snapshot is a transitional solution until the full-fledged 10.1 sensor infrastructure (Phase 11).~~ — **closed in Phase 40 Session 4**, see "Sensors" above. The snapshot itself (`cluster_node`) remains, now ALSO exposed via real sensors additively.
- ~~Rolling updates (reuse of the drain mechanism for update rollouts, expand/contract): P10-S3.~~ — **closed at P10-S3**: `registry-service`'s drain mechanism (`POST /instances/{id}/drain`/`.../activate`) + `scripts/rolling-update.sh`, see `docs/operations/rolling-updates.md`. Note this lives in `registry-service`, not here — this service's own drain-based rebalancing (3.8, P10-S2) is a separate, already-built mechanism for load-spike redistribution.
