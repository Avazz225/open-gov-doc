...
- ~~**Several render/convert/export endpoints read entire uploaded files into memory with no size cap anywhere** (watermark, convert-to-PDF, page-count, tag-check, page-image, redact, export-document, export-folder — all before PDF parsing/rasterization/a real `soffice --headless` subprocess with only a 90s timeout)~~ — **closed in Phase 61 Session 2** ([ADR 0187](../adr/0187-shared-max-upload-size-middleware.md)): `dms_common.MaxBodySizeMiddleware`, `413` above `max_upload_size_bytes` (200 MiB default), applied globally to every route via one middleware registration. Same fix shared with `virus-scan-service`/`storage-service`.
