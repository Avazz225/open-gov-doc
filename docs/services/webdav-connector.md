...
- **The remaining root-`PROPFIND` O(N) shape (the N+1 half was closed above; the "root listing costs proportional to total item count regardless of batching" half was not) has now crossed from "slow but passing" to "actually timing out"** — found during Phase 74's phase-close full regression run: 15-16 of the 18 `test_webdav.py` tests now fail with `httpcore.ReadTimeout` even at the 30s timeout above, confirmed reproducible on a second independent run (not a transient blip). Root cause: this dev stack's accumulated live-verification test data has grown to **1,639 folders / 1,675 documents** across 74+ phases, finally exceeding what even the 30s timeout tolerates for an unpaginated root listing. Unrelated to any code change in Phase 74 (no webdav-connector/folder-service/document-service diff exists that phase) — a real, worsening operational limitation, not a regression from a specific session. A real fix needs actual pagination of the root listing (not just batching the per-document metadata calls, which is already done); not attempted, flagged here for a future session with this specific symptom (test timeouts, not just "slow") as its trigger, since the original "documented, unfixed" framing understated how close this had already gotten.
