...
- **Root `PROPFIND` performance degrades with document volume** (found live during Post-Roadmap Phase 38 Session 3's full backend-suite run, unrelated to that session's RBAC work — a pure environmental/data-volume finding): listing the root folder calls `document-service` once per contained document (an N+1-style pattern), and this test environment's shared dev database had accumulated 2,472 documents directly under `root` over many prior sessions' live-verification runs — enough for a root listing to exceed the test client's timeout. Not a regression, not fixed in this session (a real performance characteristic worth a future look, e.g. a batched/paginated document-service lookup, but out of scope here).
