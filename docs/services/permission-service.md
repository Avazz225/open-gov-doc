...
- **`PUT /approval-config/{action_type}` bleibt bewusst ungegatet** (geprüft, nicht umgesetzt in **Post-Roadmap Phase 22 Session 3**, als dieser Endpunkt erstmals eine Admin-UI-Anbindung bekam, siehe `docs/services/admin-ui.md` "Vier-Augen-Einstellungen") — ein Self-Gating analog zu `POST`/`PUT /roles` (ADR 0071) hätte über ein Dutzend Testsuiten quer durchs Repo angefasst (auth-/config-/folder-/document-/migration-/workflow-service, webdav-connector rufen diesen Endpunkt direkt als Test-Infrastruktur auf, ohne `X-DMS-Principal`). Wer sich beliebig Zugang zum Gateway verschafft (z. B. über einen kompromittierten, aber ansonsten unprivilegierten Account), kann damit heute die Vier-Augen-Pflicht für JEDEN Aktionstyp systemweit umschalten — ein echtes, noch offenes Sicherheitsthema für eine künftige, dedizierte Härtungssession.
