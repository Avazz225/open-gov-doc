...
- **No system-wide write prohibition beyond the gateway** (4.8, since P6-S6): direct service-to-service write calls bypassing the gateway remain possible during maintenance mode, see ADR 0024. Scoped (no implementation) in **Post-Roadmap Phase 39 Session 2** ([ADR 0152](../adr/0152-maintenance-mode-service-to-service-enforcement-scoping.md)): a concrete inventory of the affected call sites (worst: several background poll loops with zero gateway involvement, e.g. archival disposal, retention forced-deletion, mail-triggered document creation) and a concrete recommendation (extend `libs/dms-permission-client` with `is_maintenance_active()`, mirroring `workflow-service`'s own already-working pattern) for a future build session.
