...
|---|---|---|
| `GET` | `/query/events?actor=&subject=&event_type=&since=&until=&limit=` | Structured filter API, no parser plugin needed. Role-gated (`admin.query_console` or activated superuser), result RBAC/scope-lock filtered. Response: `{events, total_before_filter, total_after_filter, superuser}`. **Since P61-S3** ([ADR 0188](../adr/0188-config-service-export-gating-search-facet-leak-and-query-limit-clamp.md)): `limit` is clamped to 100 in `_run_query`, the single entry point shared with the natural-language parser-plugin path below — previously unbounded, unlike `search-service`'s own `limit = min(limit, 100)` precedent for the identical parameter shape |
