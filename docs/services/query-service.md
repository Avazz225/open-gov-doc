...
- **`reporting-service`s Forensik-Trace hat KEINE zeilenweise Ergebnisfilterung wie `filtering.py` hier** — die Basis-Authentifizierung wurde in Post-Roadmap Phase 19 Session 7 nachgezogen ([ADR 0072](../adr/0072-archival-reporting-rbac.md)), die feingranulare Zeilenfilterung nach "sieht der Aufrufer dieses konkrete Ereignis überhaupt?" bleibt offen — zu großer Nebenschauplatz, ggf. spätere Session (`reporting-service` könnte künftig dieselbe `filtering.py`-Logik übernehmen).
