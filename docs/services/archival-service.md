...
- **rendering-service** (`RenderingClient`): `GET /renditions?document_id=&version_number=` (client-seitig auf `rendition_type == "pdf_archive"` gefiltert — kein serverseitiger Filterparameter auf rendering-service-Seite), `GET /renditions/{id}/content`. Seit **Post-Roadmap Phase 19 Session 8** ([ADR 0073](../adr/0073-ocr-rendering-virus-scan-rbac.md)) senden beide Aufrufe einen synthetischen `X-DMS-Principal: system:archival-service`-Header — rendering-service prüft seither `rendering.read`.
...
Bereits ausgesonderte, aber noch innerhalb der Übergangsfrist befindliche Dokumente/Umlaufmappen bleiben über `GET /released-items?q=` durchsuch-/einsehbar, statt nur indirekt über Audit-Trail-Verweise auffindbar zu sein (Konzept 5.6, wörtlich). Vollständige Architekturbegründung: [ADR 0055](../adr/0055-aussonderungs-zugriffsbereich-hydrated-read-only-view.md).

- **Reine gefilterte Sicht, kein neuer Datenspeicher** — `browse.build_released_items` kombiniert `repository.list_transfers(status="released")`/`list_case_transfers(status="released")` mit Live-Metadaten von `document_client.get_document`/`case_client.get_case` (Titel/`attributes["Kennzeichen"]` bzw. `name`/`vorgangsnummer`). Ein einzelner nicht mehr auflösbarer Verweis wird übersprungen (geloggt), nicht die ganze Anfrage abgebrochen — gleiches Prinzip wie `directory_federation.search_all_peers` (ADR 0054).
- **Rollen-Gate wiederverwendet** — `settings.archive_retrieval_role` (Default `dms-admin`), dieselbe Rolle, die bereits `/retrieve` und `/case-archival-transfers/{id}/package` schützt, statt eines neuen Settings (Konzept 2.5 nennt für diesen Bereich "eine dedizierte Archiv-/Registratur-Rolle" — deckungsgleich mit der bereits bestehenden Rückhol-Rolle).
- **`purge_at`** (berechnet aus `released_at + dehydration_delay_days`) nur für Dokumente — Umlaufmappen kennen keinen `dehydrated`-Status und damit keinen automatischen Zeitpunkt, an dem sie aus diesem Bereich "verschwinden" (Konzept 5.6 wörtlich, aber nur für Dokumente technisch abgebildet) — bewusst offener Punkt, siehe ADR 0055.
- **Suche** als serverseitige Substring-Filterung gegen Titel/Name + Kennzeichen/Vorgangsnummer nach der Hydrierung, kein eigener Suchindex — für die erwartete Größenordnung (nur Elemente im Übergangsfrist-Fenster) ausreichend.
- **Frontend nutzt ausschließlich bereits bestehende Aktionen** — `AussonderungPane` (User-UI, IconRail 🗄️, rollen-gegated) bietet "Rückholung" (Dokumente, ruft `/retrieve` auf) und "Paket herunterladen" (Umlaufmappen, ruft `/case-archival-transfers/{id}/package` auf) — beide Endpunkte existierten bereits vor dieser Session, waren aber nur über die reine Verwaltungstabelle im Admin-UI erreichbar.
