...
- **`xdomea.parse_abgabe_message`** (new) reads a validated 0401 message back — originally scoped to
  ...
  `skipped_document_count` field — rather than rejecting the entire package. Still deliberately bounded,
  not a claim of full third-party-XDOMEA-package generality: `Teilvorgang`/`Teilakte` nested sub-hierarchy
  and the `DokumentMitSchriftstueck` choice member remain unsupported for their own metadata (their
  nested documents are still found via the unconditional descendant search, so nothing is silently
  dropped) — see ADR 0142 for the exact scope. Looks for `Dokument` elements anywhere under a
