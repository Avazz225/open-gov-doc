...
**Verantwortung:** Aussonderung & Langzeitarchivierung von Dokumenten (5.6) — überführt Dokumente nach Ablauf der Aktivphase (Objekttyp-Frist oder manueller Trigger) verpflichtend als PDF/A (Fallback: reines PDF) in ein separates Archivziel, entfernt nach einer Übergangsfrist die Live-Speicherkopie ("Dehydrieren"), und stellt einen auditierten, rollengegateten Rückhol-Vorgang bereit. Koordiniert nur die Transfer-Mechanik — `document-service` bleibt alleinige Autorität für die Dokument-Lebenszyklusfelder.

**Konzept-Referenz:** 5.6
**Eigenes Postgres-Schema:** `archival` (Tabelle `archival_transfer`)
...
- **document-service** (`DocumentClient`): `GET .../due-for-archival`, `GET .../{id}`, `GET .../{id}/versions/{n}`, `GET .../{id}/has-active-hold`, `PUT .../{id}/archived`, `.../dehydrated`, `.../rehydrated`.
...
Keine eigenen — `document.archived`/`document.dehydrated`/`document.rehydrated` werden von `document-service` publiziert, wenn dieser Service dessen `PUT .../archived`/`.../dehydrated`/`.../rehydrated`-Endpunkte aufruft (siehe `docs/services/document-service.md`).
...
- `uv run pytest services/archival-service/tests` (**31 Tests**): `test_keystore.py`/`test_crypto.py` (Roundtrip, falscher Schlüssel, fehlender Schlüssel, frischer Nonce je Aufruf), `test_repository.py` (CRUD, aktive-Transfer-Erkennung inkl. Ausschluss terminaler Status, Dehydrierungs-Fälligkeitsfilter), `test_pipeline.py` (volle Phasenkaskade `pending → released` gegen Fake-Clients, Verharren in `locked` solange die Rendition nicht bereit ist, `failed` bei fehlgeschlagener Konvertierung/Verifikation, Verschlüsselungspfad, Dehydrierungs-Tick inkl. Legal-Hold-Blockade), `test_api.py` (Endpunkt-Verdrahtung mit gemockten externen Clients — Rollen-Gate `403`, Status-Gate `409`, erfolgreiche Rückholung inkl. Live-Upload/`mark_rehydrated`-Aufruf).
- Kein eigener Live-Docker-Smoke-Test-Abschnitt hier — siehe `PROGRESS.md` "P7-S3" für den vollständigen End-to-End-Ablauf über mehrere Services hinweg (Objekttyp mit `default_archive_after_days`, PDF/`.docx`/`.png`-Dokumente, Dehydrierung, Legal-Hold-Blockade, verschlüsselte Rückholung).

## Offene Punkte

- **Kein Rollen-/Berechtigungscheck außer bei der Rückholung** — `GET /archival-transfers`/`.../{id}` sind wie die meisten administrativen Endpunkte dieses Systems ungegated (siehe `PROGRESS.md` "Autorisierung"), nur über die Admin-UI-Navigation praktisch auf Admins beschränkt.
- **Kein Retry für `failed`-Transfers** — ein fehlgeschlagener Transfer bleibt terminal, ein neuer Anlauf bräuchte einen manuellen `POST /documents/{id}/archive-request` in `document-service`, das aber erneut denselben `get_active_transfer_for_document`-Ausschluss greifen lassen würde, solange die alte `failed`-Zeile nicht separat behandelt wird — kein Admin-UI-Bedienelement dafür in dieser Session.
...
- **XDOMEA für Umlaufmappen (case-service) ist nicht Teil dieser Session** — separat geplant als **P7-S3b**, aufbauend auf derselben Transfer-Infrastruktur (siehe [ADR 0029](../adr/0029-aussonderung-xdomea-eigenimplementierung-kdbx-plugin.md)).
