...
|---|---|---|
| `POST` | `/notifications` | Send (`channel`: `email`\|`in_app`\|`webhook`, `recipient`, `subject`, `body`) - persists first, then attempts synchronous delivery. **Always `201`**, the actual result is in the `status` field (`sent`\|`failed`\|`failed_permanent`), no HTTP error code on failed delivery. **Since P6-S6**: for `channel` `email`/`in_app`, `recipient` is first checked against real `auth-service` accounts (`GET /users`) — `400` for an unknown recipient; `webhook` remains unchecked (the target is a URL, not an identity). **Since Post-Roadmap Phase 38 Session 2**: requires `X-DMS-Principal` + `notification.write` (401/403, see "Authorization" below), and a per-recipient rate limit (`429` beyond `notification_rate_limit_max_per_recipient` within `notification_rate_limit_window_seconds`, defaults 20/60s) |
...
| `GET` | `/email-template-use-cases` | Configurable email templates (Post-Roadmap Phase 30, [ADR 0111](../adr/0111-configurable-email-templates.md)) - fixed catalog of the use cases `consumer.py` dispatches on (11 since Post-Roadmap Phase 35 Session 3), each with its known `{placeholder}`s |
...
## `workflow.task_claim.abandoned` consumer (Post-Roadmap Phase 35 Session 3, 14.2/8, [ADR 0145](../adr/0145-task-claim-reassignment-expiry-notice-and-unclaimed-team-work-attribution.md))

Third subject on the `"workflow"` stream (after `workflow.task.escalated` and `workflow.federation.inbound_received`), own durable name `notification-service-task-claim-abandoned` (same reasoning as the other shared-stream overrides above). Triggered by `workflow-service`'s new `_task_claim_expiry_poll_loop` once a `TaskClaim` has sat unrenewed longer than `claim_abandonment_threshold_hours` (default 72h) — the notification half of the feature [ADR 0121](../adr/0121-dynamic-org-hierarchy-access-grants-task-claim-and-delegation-reuse.md) scoped out ("no notifications"). Same shape as the `document.lock.reminder` handler above: a single in-app nudge to the claim holder (`event.payload.principal_id`), no separate `notify_email` concept. Payload: `{task_id, task_name, principal_id, business_key}` (process instance ID is `event.subject`) — carries a direct link to the "Vorgang" detail view in `reviewer-ui` when `reviewer_ui_public_base_url` is configured (same `build_resource_link()` mechanism as `_handle_task_escalated`).

## Configurable email templates (Post-Roadmap Phase 30, [ADR 0111](../adr/0111-configurable-email-templates.md))

`src/notification_service/templates.py` adds `EmailTemplate`-backed subject/body overrides for **all** `consumer.py` use cases (11 since Post-Roadmap Phase 35 Session 3 added `workflow.task_claim.abandoned`, see above). `resolve_template(session, use_case, recipient)` tries an exact `(use_case, domain-of-recipient)` row, then the `(use_case, NULL)` catch-all row, then returns `None`. `consumer.py`'s new `_render_or_fallback()` helper wraps every handler's notification-send call: `None` → the handler's original hardcoded subject/body, unchanged from before this feature (guaranteeing zero behavior change for any installation that has never configured a template); a resolved template's `render_template()` (`str.format()`-based) either substitutes cleanly or raises `UnknownPlaceholderError` on a typo'd placeholder, in which case the fallback is used and a warning logged — a misconfigured template can never cause a broken/half-rendered email to actually be sent. Resolution runs **per notification, not per event**: an event producing both an in-app and an email notification (e.g. `workflow.task.escalated`) resolves separately per channel's own recipient, so a domain-specific override can legitimately change only the email wording, not the in-app one (which almost never carries an `"@"` recipient and therefore only ever matches a catch-all row). `GET /email-template-use-cases` exposes the fixed catalog (with each use case's known placeholders) for the admin-ui form (`apps/admin-ui/src/app/email-templates/`, `EmailTemplates.tsx`) - deliberately a closed catalog rather than free text, since it mirrors `consumer.py`'s actual fixed set of dispatch branches (see ADR 0111 for the full rationale against `ApprovalActionConfig`'s open free-text `action_type`).

**Authorization (Post-Roadmap Phase 38 Session 3)**: `PUT /email-templates/{use_case}`, `PUT /email-templates/{use_case}/by-domain/{domain}`, and `DELETE /email-templates/{id}` previously had no permission check at all. All three now require `X-DMS-Principal` + the new capability `admin.notification_config` (role `domain-admin-notification`) — deliberately a SEPARATE domain from P38-S2's own `notification.write`: that one governs who may TRIGGER a single notification (a one-off action), this one governs who may change the wording every FUTURE notification of a given use case is sent with (an installation-wide configuration change). `GET /email-templates`/`GET /email-template-use-cases` remain ungated. See [ADR 0148](../adr/0148-admin-ui-authorization-full-alignment.md).
...
**Consumed** (`durable="notification-service"`, except where noted): `workflow.task.escalated` (from `workflow-service`), since P6-S5 additionally `auth.superuser.activated` (from `auth-service`), since P6-S6 additionally `permission.maintenance_mode.activated` (from `permission-service`, see `docs/services/permission-service.md`), since P6-S9 additionally `workflow.federation.inbound_received` (from `workflow-service`, see `docs/services/workflow-service.md` "Federation"), since **P7-S1** additionally `document.deletion.reminder` (from `document-service`, own `"document"` stream subject, see above), since **P7-S1b** additionally `folder.deletion.reminder` (from `folder-service`, own `"folder"` stream subject, see above), since **P9-S1** additionally `license.limit_exceeded`/`license.expiring_soon`/`license.invalid` (from `license-service`, each with its own durable name `notification-service-license-*`, see above), since **Post-Roadmap Phase 30 Session 4** additionally `document.lock.reminder` (from `document-service`, second `"document"` stream subject, own durable name `notification-service-lock-reminder`, see above), since **Post-Roadmap Phase 35 Session 3** additionally `workflow.task_claim.abandoned` (from `workflow-service`, third `"workflow"` stream subject, own durable name `notification-service-task-claim-abandoned`, see above).
...
- Since **Post-Roadmap Phase 35 Session 3** additionally: `test_consumer.py` (3 new tests, see [ADR 0145](../adr/0145-task-claim-reassignment-expiry-notice-and-unclaimed-team-work-attribution.md)) — the new `workflow.task_claim.abandoned` handler's fallback subject/body when no template is configured, a configured `EmailTemplate` override actually rendered, and the direct-link append/omission depending on whether `reviewer_ui_public_base_url` is configured.
- **81 tests since Post-Roadmap Phase 35 Session 3** (previously 78, +3, see above).
- Since **Post-Roadmap Phase 38 Session 2** additionally: `test_api.py` (3 new tests) — `401` without `X-DMS-Principal`, `403` without `notification.write` (a dedicated throwaway role temporarily un-assigned, since this capability is deliberately NOT part of "everyone"), and the new per-recipient rate limit (`429` on the 3rd call within the configured window with the limit temporarily lowered to 2).
- **84 tests since Post-Roadmap Phase 38 Session 2** (previously 81, +3, see above).
- Since **Post-Roadmap Phase 38 Session 3** additionally: `test_api.py` gained its first-ever HTTP-level coverage for the `EmailTemplate` endpoints (previously only exercised via `test_repository.py`'s direct repository calls, never through the actual API) — 5 new tests: a successful `PUT /email-templates/{use_case}` roundtrip, 401 without a principal header, and 403 without `admin.notification_config` for each of the three mutating endpoints.
- **89 tests since Post-Roadmap Phase 38 Session 3** (previously 84, +5, see above).
- Pure backend session, no browser test needed (the corresponding admin-ui page has its own vitest suite, see `apps/admin-ui/tests/email-templates.test.tsx`, plus a live Playwright round-trip in `apps/admin-ui/e2e/email-templates.spec.ts`); the claim-abandonment notification's own UI-visible effect (in-app notification) was live-verified in `workflow-service`'s own live smoke test, see `docs/services/workflow-service.md`.

## Authorization (Post-Roadmap Phase 38 Session 2)

`POST /notifications` previously required no permission of the *caller* at all (only that the specified `recipient` was a known `auth-service` account, see below) — any authenticated principal could trigger a notification to any known user, a spam/abuse vector. Now requires `X-DMS-Principal` + the new capability `notification.write` (401/403). Deliberately **not** added to the "everyone" group, unlike `audit.read`/`virus_scan.*` in this same session — this endpoint genuinely has only one real caller today (`reporting-service`'s scheduled-report poll loop), so it was instead given its own dedicated role grant for the fixed principal `reporting-service-scheduler` (the same identity `reporting-service` already asserts for its `audit-service` calls in the same poll tick, see `docs/services/reporting-service.md`) — mirrors `virus-scan-service`'s own `admin.quarantine` precedent (ADR 0073): a capability that was never de-facto-open-to-everyone stays that way. Also added: a per-recipient in-process sliding-window rate limit (`notification_rate_limit_max_per_recipient`/`notification_rate_limit_window_seconds` settings, default 20 per 60s, `429` beyond the limit) as defense in depth on top of the RBAC fix. Both are additive on top of the existing recipient-existence check below and apply only to the HTTP endpoint — the internal NATS consumers (`workflow.task.escalated` etc.) call `repository.create_and_send` directly and are unaffected by either change.

## Open Points

- ~~**Role check since P6-S6 only as a recipient existence check**~~ — **resolved in Post-Roadmap Phase 38 Session 2**, see "Authorization" above.
...
- ~~**No rate limiting/spam protection**~~ — **resolved in Post-Roadmap Phase 38 Session 2** (per-recipient sliding-window limit, see "Authorization" above), for the HTTP endpoint. A process with a very short, repeatedly firing cycle timer that calls `repository.create_and_send` directly (bypassing `POST /notifications`, not tested this session, see `docs/services/workflow-service.md` "Open Points") would still not be caught by this limiter.
