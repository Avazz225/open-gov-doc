...
**Verantwortung:** Notification Service (Konzept 7.1) - E-Mail-, In-App- und Webhook-Benachrichtigungen. Konsumiert `workflow.task.escalated` von `workflow-service` (SLA-Zeitüberwachung, P6-S2) sowie seit P6-S5 `auth.superuser.activated` (Break-Glass-Sicherheitsbenachrichtigung, 4.6), seit P6-S6 `permission.maintenance_mode.activated` (Not-Shutdown-Sicherheitsbenachrichtigung, 4.8), seit P6-S9 `workflow.federation.inbound_received` (Federation Hub, 7.4) und seit **P7-S1** `document.deletion.reminder` (Löscherinnerung, 5.2a, aus `document-service`), seit **P7-S1b** zusätzlich `folder.deletion.reminder` (dieselbe Löscherinnerung für Ordner, aus `folder-service`) und bietet zusätzlich einen generischen `POST /notifications`, den jeder Service direkt aufrufen kann. Seit **P6-S6** prüft dieser Endpunkt die Empfänger-Existenz gegen echte `auth-service`-Konten und bleibt bewusst auch während des systemweiten Wartungsmodus erreichbar (wird für die Alarmierung selbst gebraucht) — siehe [ADR 0024](../adr/0024-not-shutdown-gateway-enforced.md).
...
|---|---|---|
| `POST` | `/notifications` | Versand (`channel`: `email`\|`in_app`\|`webhook`, `recipient`, `subject`, `body`) - persistiert zuerst, versucht dann synchron zuzustellen. **`201` immer**, das tatsächliche Ergebnis steht im `status`-Feld (`sent`\|`failed`\|`failed_permanent`), kein HTTP-Fehlercode bei fehlgeschlagener Zustellung. **Seit P6-S6**: für `channel` `email`/`in_app` wird `recipient` zuerst gegen echte `auth-service`-Konten geprüft (`GET /users`) — `400` bei unbekanntem Empfänger; `webhook` bleibt ungeprüft (Ziel ist eine URL, keine Identität) |
| `GET` | `/notifications?recipient=&channel=&status=` | Gefilterte Liste |
| `GET` | `/notifications/{id}` | Detail - `404` |
| `POST` | `/notifications/{id}/retry` | Manueller Neustart einer `failed_permanent`-Notification (seit **P20-S3**, [ADR 0079](../adr/0079-notification-service-retry-backoff-failed-permanent.md)) — `404` bei unbekannter `id`, `409` wenn `status != "failed_permanent"`, sonst sofortiger erneuter Zustellversuch |
| `GET` | `/healthz` | Health-Check |

## Datenmodell

`notification`: `id`, `channel` (`"email"`\|`"in_app"`\|`"webhook"`), `recipient` (heterogen je Kanal: E-Mail-Adresse / Lane-Name-oder-Nutzerkennung / Webhook-URL - bewusst ein einziges generisches Feld statt dreier kanalspezifischer Spalten), `subject`, `body`, `status` (`"sent"`\|`"failed"`\|`"failed_permanent"`, seit **P20-S3**), `error` (nullable), `attempts` (Integer, Default 0, seit **P20-S3**), `next_retry_at` (nullable, seit **P20-S3**), `created_at`, `sent_at` (nullable).
...
- **In-App**: reine Persistenz, sofort `status="sent"`. Kein UI diese Session (Abfrage nur über `GET /notifications`) - Konzept 7.1 nennt In-App als Kanal, eine UI-Anzeige ist nicht Teil von P6-S2. Nie retry-fähig (kein echter Zustellschritt, kann nicht fehlschlagen).
- **Webhook**: HTTP-POST via `httpx` an die als `recipient` übergebene URL, JSON-Payload `{"subject": ..., "body": ...}`, Timeout 5s.

Der ERSTE Zustellversuch passiert weiterhin **synchron** beim Anlegen des Datensatzes (schnelle Antwort im Regelfall), Ergebnis landet direkt am selben Datensatz. **Seit Post-Roadmap Phase 20 Session 3** ([ADR 0079](../adr/0079-notification-service-retry-backoff-failed-permanent.md)) ist ein Fehlschlag bei `email`/`webhook` nicht mehr sofort terminal: `status` bleibt `"failed"` (retry-fähig) mit steigendem `attempts` und einem per Full-Jitter-Backoff (`libs/dms-retry`) gesetzten `next_retry_at`, den ein neuer, eigenständiger `_notification_retry_poll_loop` (Intervall `notification_retry_poll_interval_seconds`, Default 60s) abarbeitet — die Zustellung selbst bleibt synchron/inline, nur die WIEDERHOLUNG läuft asynchron, um den NATS-Handler nicht zu blockieren. Erst nach `max_notification_attempts` (Default 5) erfolglosen Versuchen wechselt `status` auf das echte Terminalstatus `failed_permanent`, ab dem `POST /notifications/{id}/retry` einen sofortigen manuellen Neustart erlaubt.
...
## `document.deletion.reminder`-Konsument (seit P7-S1, 5.2a)

Fünfter Zweig desselben `consumer.py`-Handlers, eigenes Stream-Subject (`"document"`, siehe `settings.subjects`) — keine Kollisionsgefahr mit den bisherigen `workflow.>`-Subjects, da es sich um einen eigenen Stream handelt. Gleiches `notify_email`-Muster wie beim Federation-Zweig: **immer** eine In-App-Notification (Empfänger `"unassigned"` — keine Rollen-/Nutzer-Auflösung, dasselbe bewusste Vereinfachungsmuster wie überall sonst in diesem Service), zusätzlich eine E-Mail, falls das Event ein optionales `notify_email`-Feld enthält (`Document.reminder_notify_email`, vom Aufrufer beim Setzen der Aufbewahrung mitgegeben, sonst keine E-Mail). Ausgelöst vom `_retention_poll_loop` des `document-service`, sobald ein Dokument innerhalb der konfigurierten Vorlaufzeit (`RetentionConfig.reminder_lead_days`) zur Löschung ansteht (siehe `docs/services/document-service.md` "Aufbewahrung & Legal Hold").
...
## `folder.deletion.reminder`-Konsument (seit P7-S1b, 5.2a)

Sechster Zweig desselben `consumer.py`-Handlers, 1:1 dasselbe Muster wie `document.deletion.reminder` (s. o.), nur `folder-service`s `name`-Feld statt `title` im Payload. Eigenes, bisher nicht von diesem Service konsumiertes `"folder"`-Stream-Subject — da dies das **erste** Subject dieses Service auf diesem Stream ist, war (anders als bei `workflow.federation.inbound_received`) **kein** zweiter Durable-Name nötig, der oben beschriebene Bug trat hier von vornherein nicht auf.

## `license.*`-Konsumenten (seit P9-S1, 9.2)

Drei weitere Zweige desselben `consumer.py`-Handlers (`license.limit_exceeded`/`license.expiring_soon`/`license.invalid`), 1:1 dasselbe Muster wie `_handle_maintenance_mode_activated` — feste `settings.license_admin_email`-Adresse, kein Empfänger-Auflösungsmechanismus. Da alle drei Subjects den neuen `"license"`-Stream teilen (anders als `document.deletion.reminder`/`folder.deletion.reminder`, die jeweils das **erste** Subject ihres Streams waren), brauchte **jedes** einen eigenen Durable-Namen (`notification-service-license-limit-exceeded`/`-expiring-soon`/`-invalid`) — derselbe Durable-Name für mehrere Filter-Subjects auf demselben Stream schlägt mit "consumer is already bound to a subscription" fehl, dieselbe Einschränkung, die bereits `workflow.federation.inbound_received` einen zweiten Durable-Namen abverlangt hatte.
...
**Konsumiert** (`durable="notification-service"`, außer wo vermerkt): `workflow.task.escalated` (aus `workflow-service`), seit P6-S5 zusätzlich `auth.superuser.activated` (aus `auth-service`), seit P6-S6 zusätzlich `permission.maintenance_mode.activated` (aus `permission-service`, siehe `docs/services/permission-service.md`), seit P6-S9 zusätzlich `workflow.federation.inbound_received` (aus `workflow-service`, siehe `docs/services/workflow-service.md` "Federation"), seit **P7-S1** zusätzlich `document.deletion.reminder` (aus `document-service`, eigenes `"document"`-Stream-Subject, siehe oben), seit **P7-S1b** zusätzlich `folder.deletion.reminder` (aus `folder-service`, eigenes `"folder"`-Stream-Subject, siehe oben), seit **P9-S1** zusätzlich `license.limit_exceeded`/`license.expiring_soon`/`license.invalid` (aus `license-service`, je eigener Durable-Name `notification-service-license-*`, siehe oben).
...
`uv run pytest services/notification-service/tests` (**40 Tests**, davon 10 neu seit **Post-Roadmap Phase 20 Session 3**, [ADR 0079](../adr/0079-notification-service-retry-backoff-failed-permanent.md)) - läuft gegen eine echte Postgres-Instanz, echten NATS und (für die E-Mail-Pfade) gegen den echten `mailpit`-Container, kein Mocking:
...
- `test_repository.py` - `create_and_send` inkl. Persistenz des Fehlerfalls, Filterung nach `recipient`/`channel`. Seit P20-S3 zusätzlich: Backoff-Verhalten unterhalb/bei Erschöpfung von `max_notification_attempts`, `list_due_for_retry`-Filterung (Status UND Backoff-Fenster), `retry_now`.
- `test_api.py` - alle Endpunkte inkl. `404`. Seit P20-S3 zusätzlich: neuer `/retry`-Endpunkt inkl. `404`/`409`/erfolgreichem Neustart.
- `test_main.py` (neu seit P20-S3) - `_run_retry_tick` greift eine fällige Notification auf und versucht sie erneut zuzustellen, überspringt eine noch nicht fällige.
...
- Seit **P7-S1** zusätzlich: `test_consumer.py` simuliertes `document.deletion.reminder`-Event mit/ohne `notify_email` (In-App+E-Mail bzw. nur In-App).
- Seit **P7-S1b** zusätzlich: `test_consumer.py` simuliertes `folder.deletion.reminder`-Event mit `notify_email` (In-App+E-Mail).
- **27 Tests** (vorher 26, 1 neu: siehe oben).
...
- ~~**Kein Retry/keine Dead-Letter-Behandlung**~~ — **behoben in Post-Roadmap Phase 20 Session 3** ([ADR 0079](../adr/0079-notification-service-retry-backoff-failed-permanent.md)): automatischer Retry mit Full-Jitter-Backoff bis `max_notification_attempts`, danach `failed_permanent` + manueller Neustart über `POST .../retry`. ~~Noch offen: eine Admin-UI-Sichtbarkeit/Bedienelement dafür~~ — **behoben in Post-Roadmap Phase 20 Session 7** ([ADR 0083](../adr/0083-admin-ui-processing-failures-visibility.md)): `ProcessingFailuresView` (`/processing-failures/`) in `admin-ui`.
- **Kein Retrofit bestehender "loggt nur"-Alarmierungsstellen** - `storage-service` (und andere, in Konzept an verschiedenen Stellen erwähnte künftige Konsumenten wie Force-Unlock, Lizenz-Ablauf, Report-Versand, Monitoring-Eskalation) bleiben **nicht** auf den Service umgehängt. Break-Glass (4.6, seit P6-S5) und **Löschfrist-Vorankündigung (5.2a, seit P7-S1)** sind die bisherigen Ausnahmen (siehe oben). Jeder künftige Konsument trägt sein Subject selbst in `settings.py`s `subjects`-Liste ein, sobald er tatsächlich angebunden wird.
