...
|---|---|---|
| `POST` | `/scan` | Multipart (`file`, optional `document_id`/`created_by`) → performs the scan, on a hit stores a quarantine copy in the Storage Service, persists and returns the result (`ScanResultOut`). Requires `X-DMS-Principal` + `virus_scan.write` since **Post-Roadmap Phase 38 Session 2**, see "Authorization" below. **Since Phase 61 Session 2** ([ADR 0187](../adr/0187-shared-max-upload-size-middleware.md)): `413` for a request whose declared `Content-Length` exceeds `max_upload_size_bytes` (200 MiB default) — previously read the entire file into memory with no size cap at all. |
...
- ~~**`POST /scan` read the entire uploaded file into memory with no size cap anywhere**~~ — **closed in Phase 61 Session 2** ([ADR 0187](../adr/0187-shared-max-upload-size-middleware.md)): `dms_common.MaxBodySizeMiddleware`, `413` above `max_upload_size_bytes` (200 MiB default), rejects based on `Content-Length` before routing/parsing even starts. Same fix shared with `rendering-service`/`storage-service` (the identical gap independently in all three).
