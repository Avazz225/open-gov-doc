...
// Cross-UI theming (8, P4-S6): light/dark/high-contrast/automatic,
// cross-device via the user profile (ADR 0009). `localStorage` remains as
// an immediately available cache so that a later login doesn't have to
// wait for the first server response.
export function ThemeProvider({ children }: { children: ReactNode }) {
  const { accessToken } = useAuth();
  const [theme, setThemeState] = useState<ThemeName>(() => loadCachedTheme());

  // `useLayoutEffect` instead of `useEffect`, so that the attribute is set
  // before the first visible paint (no flash of the wrong theme).
  useLayoutEffect(() => {
  ...
  useEffect(() => {
    ...
    getThemePreference(accessToken)
      ...
      .catch(() => {
        // Deliberately silent: the locally cached value remains valid if
        // the server preference is not (yet) readable.
      });
  }, [accessToken]);
    ...
    (next: ThemeName) => {
      ...
      if (accessToken) {
        updateThemePreference(accessToken, next).catch(() => {
          // Selection remains valid locally immediately, even if server
          // persistence fails - no retry in this scaffold.
        });
      }
    },
    [accessToken]
