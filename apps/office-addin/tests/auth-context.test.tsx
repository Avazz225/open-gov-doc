...
describe("AuthProvider (office-addin)", () => {
  beforeEach(() => {
  ...
  it("login stores the session under the ogdoc-specific storage key", async () => {
    ...
    expect(window.localStorage.getItem("ogdoc.tokens")).not.toBeNull();
  });
  ...
  it("logout clears the stored session", async () => {
    ...
    expect(screen.getByTestId("user").textContent).toBe("none");
    expect(window.localStorage.getItem("ogdoc.tokens")).toBeNull();
  });
