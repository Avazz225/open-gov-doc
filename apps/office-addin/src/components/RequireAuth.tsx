...
// Static export has no server that could perform redirects before
...
// (identical pattern to reviewer-ui/migration-console).
export function RequireAuth({ children }: { children: ReactNode }) {
  ...
  if (isLoading) {
    return <p className="p-3">{t("common.loading")}</p>;
  }
  if (!user) {
    return null;
  }
