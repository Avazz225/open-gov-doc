...
// Cross-UI theming (8, P4-S6): light/dark/high-contrast/auto,
// cross-device via the user profile (ADR 0009). `localStorage` remains
// as an immediately available cache, so that a later login doesn't have to
    ...
    (next: ThemeName) => {
      ...
      if (accessToken) {
        updateThemePreference(accessToken, next).catch(() => {
          ...
          // server persistence fails - no retry in this scaffold.
        });
      }
    },
    [accessToken]
