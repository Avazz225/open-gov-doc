...
describe("LocaleProvider (process-designer)", () => {
  beforeEach(() => {
  ...
  it("restores a cached locale from localStorage after mount, not as the initial render", () => {
    // Regression test for a real hydration bug found live during this
    // session's own end-to-end verification (React error #418): unlike
    // `ThemeProvider`, which can safely read `localStorage` as the initial
    // `useState` value (theme only drives an imperative attribute), the
    // static export always server-renders `defaultLocale`'s strings -
    // seeding the initial state from a cached non-default locale would
    // mismatch that markup on hydration. The cached value is applied in a
    // `useEffect` instead, always one tick after the first render.
