#!/usr/bin/env bash
# Copy of infra/postgres-init/002-service-roles.sh (P68-S1) - Helm's
# `.Files.Glob` can only read files inside the chart directory, so a second
# copy lives here instead of a reference to infra/postgres-init/, same
# convention as 001-schemas.sql's own copy in this directory. Must be kept
# manually in sync if infra/postgres-init/002-service-roles.sh changes.
# Mounted at /docker-entrypoint-initdb.d in the bundled Postgres Deployment
# (templates/postgresql.yaml) via ConfigMap "<fullname>-postgresql-init" -
# each POSTGRES_PASSWORD_<SCHEMA> env var it reads is injected into that
# same Deployment's own env, sourced from the per-service secrets
# templates/secrets.yaml generates (see dms.postgresServiceSecretName).
#
